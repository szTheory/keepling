...
const runLane = ({ args, command, cwd, env, name, parse, trackedInputPaths }) => {
  ...
  if (!passed) {
    ...
    // was terminated mid-run reported its parse failure instead, which reads
    // as "the work did not pass" when what happened is "the work never
    // finished". Those must never be indistinguishable: the first is a defect
    // in the code under test, the second a defect in this runner. Measured in
    // the iOS gate, window 102; fixed here at the same time because this
    // runner had the identical unset-maxBuffer spawnSync.
