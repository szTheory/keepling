...
workspace_input=$2
credential=${KEEPLING_BACKUP_PRIMARY_CREDENTIAL_FILE:-}
selection=${KEEPLING_BACKUP_PRIMARY_SELECTION_FILE:-}
recovery_external_file KEEPLING_BACKUP_PRIMARY_CREDENTIAL_FILE "$credential" "$repository_root"
recovery_external_file KEEPLING_BACKUP_PRIMARY_SELECTION_FILE "$selection" "$repository_root"
recovery_validate_credential "$credential"
recovery_validate_selection "$selection" b2-primary
...
if [ -n "${KEEPLING_BACKUP_PRIMARY_FIXTURE_ROOT:-}" ]; then
  root=$KEEPLING_BACKUP_PRIMARY_FIXTURE_ROOT
  ...
  recovery_validate_head "$metadata" b2-primary "$ciphertext_sha"
  ...
  printf '%s\n' head >&2; recovery_validate_head "$metadata" b2-primary "$ciphertext_sha"
  AWS_ACCESS_KEY_ID=$(jq -r '.access_key_id' "$credential") AWS_SECRET_ACCESS_KEY=$(jq -r '.secret_access_key' "$credential") AWS_DEFAULT_REGION=$region \
...
printf '%s\n' package-manifest >&2
if [ -n "${KEEPLING_BACKUP_PRIMARY_FIXTURE_ROOT:-}" ]; then fixture_boundary package-manifest; fi
...
if [ -n "${KEEPLING_BACKUP_PRIMARY_FIXTURE_ROOT:-}" ]; then fixture_boundary decrypt; fi
"$cipher" "${KEEPLING_BACKUP_CIPHER_FILE:-}" "$encrypted" "$plain"
printf '%s\n' decrypt >&2
[ -f "$plain" ] && [ ! -L "$plain" ] || die "decrypt did not create plaintext"
recovery_publish "$workspace" b2-primary "$ciphertext_sha" "$plaintext_sha" "$(wc -c <"$archive" | tr -d '[:space:]')" "$plain"
trap - EXIT HUP INT TERM
rm -f -- "$metadata" "$archive" "$encrypted" "$workspace/.recovery-manifest"
echo "Recovery source verified: b2-primary"
