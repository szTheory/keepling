...
- [ ] **MAC-03**: User can mutate tasks while offline, quit or lose the process, relaunch, and later reconcile without losing or duplicating accepted intent.  <!-- RE-DERIVED 2026-09-04 after O-34 closed. The old blocker is GONE: 03-21 proved the PACKAGED app (no KEEPLING_TEST_SYNC_MODE, no stubbed fetch) captures while unreachable, retries the EXACT bytes, and reconciles against real Phoenix on real PostgreSQL -- byte-compared against what the SERVER received via a forwarding proxy, not against the client's claim, with the server itself returning 404 for the mutation while offline. Durability across hard kill and relaunch was already proven. STILL NOT CHECKED, for a new and narrower reason -- O-41: `type: 'capture_task'` (DesktopApplication.ts:259) is the ONLY command type desktop production code ever constructs. editTask, applyLifecycle and moveToday write the local projection and enqueue nothing; there is no production enqueue call site at all. The requirement says `mutate tasks ... and later reconcile`, and completing, reopening, trashing, restoring, editing a title and moving to Today never reach the server. CAPTURE reconciliation is fully proven; the other mutations are not implemented on the wire. Check when they enqueue, or narrow the requirement with a recorded decision -- not before. -->
- [ ] **MAC-04**: User can inspect offline, syncing, conflict, authentication-expired, and unrecoverable states without reading logs.  <!-- UN-CHECKED 2026-09-04, self-correction. It was checked on the strength of the note below, which asserted "All five states are constructed AND visible." That is FALSE for conflict, and I wrote it. O-38: the union variant { kind: 'conflict' } (presentation.ts:40) has authored copy, a Review Conflict action and a unit-test fixture, and NO production construction site -- the exact O-30 shape, found by the O-33 hand-audit that O-30 was supposed to have motivated. It is unreachable by EVERY path, including the test stub: index.ts:359 mints a conflict OUTCOME and local-store.ts:385 builds a conflict record from it, but nothing anywhere publishes the conflict PRESENTATION. Worse, it is not merely unwired but unreachable BY CONSTRUCTION: the server genuinely persists conflicts (command_store.ex:707) and the contract's outcome enum carries conflict and rejected (keepling.yaml:1392), while the real adapter's mapAcknowledgement (adapters/sync.ts:90-94) accepts ONLY accepted/already_satisfied and throws 'server acknowledgement is invalid' on both. A real conflict from a real server crashes the adapter. This was latent only because no packaged path had ever reached a real server; closing O-37 makes it live. Re-check ONLY when a real server conflict is surfaced as conflict copy through the shipped window. Offline, syncing, authentication-expired and unrecoverable remain proven and are unaffected. -->
...
| MAC-01, MAC-02, QUAL-04 | Phase 3 | Complete — gate lanes=9 failed=0, macOS lane rows=15 failed=0 cases=92 |
| MAC-04 | Phase 3 | CORRECTED — four of five states proven; conflict is unreachable by construction (O-38). The real adapter throws on the conflict and rejected outcomes the server actually emits and the contract publishes |
| MAC-03 | Phase 3 | Blocked on O-41 — packaged real-server capture reconciliation fully proven; edits, lifecycle and Today moves enqueue nothing and never reach the server |
