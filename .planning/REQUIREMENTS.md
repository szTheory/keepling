...
- [ ] **MAC-02**: User can perform the supported daily loop with complete keyboard navigation and quick entry.  <!-- re-derived 2026-09-04: the ORIGINAL blocker is gone -- 03-06 Task 3 was superseded by 03-15 and rows A1-A15 are automated (15 rows / 92 cases, five consecutive --all runs at rows=15 failed=0), plus 10 keyboard E2E cases. STILL UNCHECKED for a NEW and real reason: O-36. After dismissing the Quick Entry discard confirmation with 'Keep Draft', AX focus lands on the AXWebArea rather than a control, and because quick-entry.tsx binds Escape via onKeyDown inside the React root the key never reaches it -- Escape SILENTLY stops working until the user tabs back into the field. Reproduced twice with a standalone AXProbe. 'Complete keyboard navigation' is not true while that holds. -->
- [x] **MAC-03**: User can mutate tasks while offline, quit or lose the process, relaunch, and later reconcile without losing or duplicating accepted intent.  <!-- CHECKED 2026-09-04: the stated blocker (O-16, sync not wired) was closed by 03-14 -- bootstrap() constructs the real KeeplingSyncAdapter and the stub survives only behind KEEPLING_TEST_SYNC_MODE. Evidence: test/packaged/daily-loop.spec.ts proves an offline capture and a durable Quick Entry draft both survive a hard kill and relaunch of the PACKAGED app then reconcile with exactly one reconnect; test/packaged/offline-capture.spec.ts proves offline capture survives hard kill and exact acknowledgement; test/e2e/real-stack-sync.spec.ts proves against a REAL server that a receipt for a different mutation identity never settles anything and never claims Synced. Exact serialized command bytes are retried, never re-serialized. -->
...
- [ ] **QUAL-03**: Release promotion uses the exact revision and artifact previously tested; distribution jobs do not silently rebuild different bytes.  <!-- re-derived 2026-09-04. FIRST CLAUSE PROVEN: package-once produces an immutable manifest, smoke consumes only that manifest and refuses rebuild/source-tree/dev-server paths, and the build was MEASURED reproducible on 2026-09-04 (two consecutive packages -> identical applicationDigestSha256 937b279c and executableDigestSha256 cb1ff051). SECOND CLAUSE UNPROVEN: "distribution jobs do not silently rebuild" is a claim about CI, and no workflow in .github/workflows/ has ever executed because this repository has no git remote (O-35). Left unchecked pending an explicit decision: check it with this disclosure, or leave it for the Phase 6 cross-system verification the traceability table already assigns. -->
...
| MAC-01, MAC-03, MAC-04, QUAL-04 | Phase 3 | Complete — gate lanes=9 failed=0, macOS lane rows=15 failed=0 cases=92 |
| MAC-02 | Phase 3 | Blocked on O-36 (Escape silently dead after the Quick Entry discard confirmation) |
| MAC-05 | Phase 3 | Blocked on O-34 (no packaged test contacts a real server) |
| QUAL-03 | Phase 3 | Local digest-bound promotion proven and the build measured reproducible; the "distribution jobs" clause is a CI claim and CI has never run (O-35) |
| SRV-02 Electron adapter proof | Phase 3 | Electron adapter proven against a real server in test/e2e/real-stack-sync.spec.ts; SRV-02 completes at the Phase 5 cross-adapter proof |
