...
- [x] **MAC-01**: User can capture, view Inbox and Today, edit, complete, reopen, trash, restore, and undo from the Electron Mac client. <!-- RE-CHECKED 2026-09-04 (post-gap-closure verification, 03-VERIFICATION.md 2026-09-04T22:10:00Z). Commit 9bde015 unchecked ALL eight Phase 3 requirement IDs under the revert-on-gaps-found convention; that revert was scoped to the phase, not to this ID specifically. Neither closed gap (Gap 1 macOS-lane evidence, Gap 2 UI-SPEC evidence contract) touched this requirement substance. Re-verification re-ran the full desktop gate fresh at HEAD 3853261: lanes=11 failed=0, electron-e2e cases=76, real-stack-sync cases=5, packaged cases=11, ipc-hostile-bridge cases=75, macos-integration cases=93. No code or test change was required to restore this checkbox. -->
...
- [x] **MAC-03**: User can mutate tasks while offline, quit or lose the process, relaunch, and later reconcile without losing or duplicating accepted intent.  <!-- CHECKED 2026-09-04, last requirement of the phase, after three blockers fell in sequence. O-34: the packaged app (no KEEPLING_TEST_SYNC_MODE, no stubbed fetch) exchanges real bytes with real Phoenix on real PostgreSQL. O-41: all eight desktop command types were asserted BY NAME arriving at the server; ordering is enforced by a task-scoped resource key and proven by ARRIVAL ORDER via the forwarding proxy, not by a journal dependency, which would have stranded a task chain once conflicts became reachable. O-45: undo reconciles through the server-issued handle. O-51: an undo of bytes the server never received drops the command from the outbox instead, so Command-Z works on a Mac with no server. "Without duplicating" is structural, not asserted -- outbox transmission state is MONOTONIC (queued -> in_flight -> uncertain, never backwards, migrations/0002_outbox_state.sql), so a command whose fate is unknown is re-pushed under the server idempotency key and can never be dropped and re-created. "Without losing" is proven across hard kill and relaunch AND across the 0002 migration, by the SHIPPED artifact opening a genuine pre-migration database and retaining both its task and its queued command with exact bytes. Reconciliation is read from the SERVER (/api/v1/tasks/:id, /api/v1/trash, /api/v1/inbox, and the proxy record of received bodies), never from the client "Synced" label. DISCLOSURE: against the real server the observed outcomes are accepted and conflict. A server rejection and an expired undo handle are proven at the unit boundary against the server own problem bodies, not in the real-stack lane. A real mid-POST process crash is proven by close-while-in_flight-then-reopen rather than by an actual kill. Gate: lanes=10 failed=0. --> <!-- RE-CHECKED 2026-09-04 (post-gap-closure verification, 03-VERIFICATION.md 2026-09-04T22:10:00Z). Commit 9bde015 unchecked ALL eight Phase 3 requirement IDs under the revert-on-gaps-found convention; that revert was scoped to the phase, not to this ID specifically. Neither closed gap (Gap 1 macOS-lane evidence, Gap 2 UI-SPEC evidence contract) touched this requirement substance. Re-verification re-ran the full desktop gate fresh at HEAD 3853261: lanes=11 failed=0, electron-e2e cases=76, real-stack-sync cases=5, packaged cases=11, ipc-hostile-bridge cases=75, macos-integration cases=93. No code or test change was required to restore this checkbox. -->
- [x] **MAC-04**: User can inspect offline, syncing, conflict, authentication-expired, and unrecoverable states without reading logs.  <!-- RE-CHECKED 2026-09-04 after being un-checked hours earlier, now on evidence for all five named states. CORRECTION TO MY OWN EARLIER NOTE: I wrote that the contract published conflict and rejected in the acknowledgement outcome enum, citing keepling.yaml:1392. That was WRONG. CommandAcknowledgement.outcome publishes exactly accepted and already_satisfied (keepling.yaml:2233-2237); line 1392 is MutationTrustState, a CLIENT vocabulary. The real server answers a conflict with HTTP 409 and an RFC 9457 problem. The defect was therefore real but WORSE than I filed it: a 409 threw a plain Error that runSyncPass caught as a transport failure, so a person read "Couldn’t reach the server" and was offered a Retry that would retry an already-decided command forever. Misheard, not unheard. FIXED by 03-22 without widening mapAcknowledgement -- classification happens in the transport from a closed code list (main/adapters/server-refusal.ts:113), and a 200 body claiming conflict is still refused loudly, pinned by a test. The conflict was produced by a REAL second writer: a real browser client editing the same task through the real server while the packaged app was disconnected. O-42 also closed, so the authored remedies are rendered and dispatch rather than crossing IPC unread -- "inspect without reading logs" now includes being able to act. Offline, syncing, authentication-expired and unrecoverable were already proven. Remaining unreachable variants (preparing O-46, uncertain O-47) are NOT among the five states this requirement names. --> <!-- RE-CHECKED 2026-09-04 (post-gap-closure verification, 03-VERIFICATION.md 2026-09-04T22:10:00Z). Commit 9bde015 unchecked ALL eight Phase 3 requirement IDs under the revert-on-gaps-found convention; that revert was scoped to the phase, not to this ID specifically. Neither closed gap (Gap 1 macOS-lane evidence, Gap 2 UI-SPEC evidence contract) touched this requirement substance. Re-verification re-ran the full desktop gate fresh at HEAD 3853261: lanes=11 failed=0, electron-e2e cases=76, real-stack-sync cases=5, packaged cases=11, ipc-hostile-bridge cases=75, macos-integration cases=93. No code or test change was required to restore this checkbox. -->
- [x] **MAC-05**: User receives proof that the packaged installed application—not only a development renderer—preserves and synchronizes the local store.  <!-- CHECKED 2026-09-04 WITH DISCLOSURE. The clause this requirement exists to force -- `not only a development renderer` -- is now genuinely satisfied. O-34 is closed: 03-21 added a real-stack gate lane in which the PACKAGED .app, with no KEEPLING_TEST_SYNC_MODE and no stubbed fetch, authorizes through a real device grant against real Phoenix on real PostgreSQL 18.6, captures while unreachable, retries the exact serialized bytes, and settles Synced on an exact receipt; both titles were read back out of real PostgreSQL through /api/v1/inbox. Namespace authority proved real rather than nominal: the client was configured with 127.0.0.1:4103 and the server-supplied origin was http://localhost:4102. `preserves` was already proven packaged. Non-vacuity measured: the lane failed four times during construction for four distinct real reasons, and pointing the app at a dead port still makes it fail. DISCLOSURE, so the checkmark is not read wider than the evidence: the only outcome ever observed is `accepted`, so this proves no conflict or rejection handling (O-38), and the only command type that can reach the wire is capture_task (O-41), so `synchronizes the local store` is proven for CAPTURES and for no other mutation. Gate: lanes=10 failed=0 including real-stack-sync. -->
...
- [x] **IOS-04**: User can distinguish local, syncing, conflict, authentication-expired, and unrecoverable states on a physical iPhone.
...
- [x] **QUAL-03**: Release promotion uses the exact revision and artifact previously tested; distribution jobs do not silently rebuild different bytes.  <!-- CHECKED 2026-09-04 WITH DISCLOSURE. Clause 1 PROVEN: tooling/package-desktop.mjs emits an immutable manifest; tooling/smoke-desktop-packaged.mjs consumes only that manifest and refuses rebuild, source-tree and dev-server paths; the nine-lane gate binds macOS row evidence to applicationDigestSha256 AND executableDigestSha256 AND the probe source digests AND the lane source digest, so stale evidence is refused rather than reused. The build was MEASURED reproducible 2026-09-04 -- two consecutive `pnpm package:desktop` on an unchanged tree produced identical applicationDigestSha256 937b279c and executableDigestSha256 cb1ff051 -- which is the property the whole cache rests on and had never previously been tested. Clause 2 ("distribution jobs do not silently rebuild") is NOT proven and is disclosed rather than claimed: no workflow in .github/workflows/ has ever executed, because this repository has no git remote (O-35). The workflow is written build-once/download-many and desktop-promote consumes only the uploaded artifact, but that is unexecuted code. Checked because the traceability table already assigns QUAL-03 its final cross-system verification to Phase 6, and the mechanism this phase owns is proven; Phase 6 must re-verify clause 2 against a real remote before any release claim.  CORRECTION 2026-09-04 (O-40): an earlier note in this session described the packaged build as measured reproducible. That over-generalised the evidence. Two back-to-back `pnpm package:desktop` invocations in ONE shell do produce identical digests -- that is what was actually measured -- but three SEPARATE invocations at the same clean revision (fc3ac82) produced three different applicationDigestSha256 values. The digest binding in verify-macos-integration.mjs is CORRECT and must not be loosened to accommodate this; the packaging is what needs fixing. Promotion-refuses-rebuild remains fully proven and is unaffected. RE-CHECKED 2026-09-04 by 03-26: this checkbox had been reverted to unchecked pending VERIFICATION.md Gap 1's macOS `--all` recording-run closure (03-25 closed the packaging-reproducibility half but did not itself flip this checkbox -- an O-22-style gap, not a fresh finding). 03-25 measured `differing_entries=0` across three separate builds, and 03-26 closed the remaining half: a full A1-A15 recording now passes on two consecutive runs plus one seeded shuffled order, and `verify-desktop-phase.mjs` reuses that evidence cleanly across two consecutive gate runs. Clause 2 (distribution jobs / CI) remains explicitly unproven and deferred to Phase 6 exactly as already disclosed above. -->
- [x] **QUAL-04**: Important screens have representative user-level coverage for meaningful populated, empty, loading, offline, denied, stale, conflict, partial, retry, and unrecoverable states.  <!-- CHECKED 2026-09-04: the stated blocker (03-06 Task 3) was superseded by 03-15. Evidence: test/application/state-matrix.test.tsx covers the state vocabulary; test/e2e/accessibility.spec.ts covers semantic structure, focus, 200% reflow, themes and motion; the macOS lane measures appearance and computed WCAG contrast from captured pixels against the packaged artifact. Materially, 03-19 made these states VISIBLE for the first time -- until then the rows were published to a renderer that rendered none of them, so this could not honestly have been checked before that plan regardless of the stated reason. RE-CHECKED 2026-09-04 by 03-27, closing VERIFICATION.md Gap 2: the desktop visual-snapshot evidence dimension (five window sizes, light/dark) named in 03-UI-SPEC.md's Verification Evidence Required section had no executable artifact and had been signed off PASS anyway. 03-27 built apps/desktop/test/e2e/appearance-matrix.spec.ts (ten behavioral cases proving the five sizes in both themes against a real resized window) and amended 03-UI-SPEC.md with a per-dimension evidence table naming a test title or macOS row id (or NOT BUILT with evidence, for the still-unbuilt responsive drawer composition) for every dimension, plus a dated correction beneath the Checker Sign-Off. `node tooling/verify-desktop-phase.mjs` reports `lanes=11 failed=0` on two consecutive runs with electron-e2e at cases=76 (up from 66). -->
...
| MAC-03 | Phase 3 | Complete with disclosure — all eight command types plus undo reconcile against a real server; no-duplication is structural via monotonic outbox state; rejection and handle expiry proven at the unit boundary only |
| MAC-05 | Phase 3 | Complete with disclosure — packaged .app exchanged real bytes with real Phoenix on real PostgreSQL; proven for captures and the accepted outcome only (O-38, O-41) |
| QUAL-03 | Phase 3 | Complete with disclosure — local digest-bound promotion proven. CORRECTED 2026-09-04: the build is NOT measured reproducible (O-40) — the earlier measurement showed only same-shell determinism and was over-generalised; three digests at one clean revision across separate invocations. The "distribution jobs" clause is unexecuted CI (O-35), re-verify in Phase 6 |
| SRV-02 Electron adapter proof | Phase 3 | UPDATED 2026-09-04 — O-34 closed: the Electron adapter HAS now contacted a real server from the packaged app, for capture_task and the accepted outcome. SRV-02 still completes at the Phase 5 cross-adapter proof |
