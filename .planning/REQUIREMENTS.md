...
- [ ] **MAC-03**: User can mutate tasks while offline, quit or lose the process, relaunch, and later reconcile without losing or duplicating accepted intent.  <!-- RE-DERIVED 2026-09-04 against 03-22. O-41 is CLOSED and the bulk of this requirement is now genuinely proven: all eight desktop command types were asserted BY NAME arriving at the real server, ordering is enforced by a task-scoped resource key (deliberately not a journal dependency, which would have stranded a task’s whole chain in the outbox once conflicts became reachable) and proven by ARRIVAL ORDER at the server via the forwarding proxy, and reconciliation is read back from the server (/api/v1/tasks/:id, /api/v1/trash, /api/v1/inbox, final_revision=8 being the server’s own count), never from the client’s "Synced" label. 03-22 also found and fixed computeSyncBackoff being defined, exported and CALLED FROM NOWHERE -- an app that reconnected never synced until the person typed something. STILL NOT CHECKED, on O-45: undoLastLocalAction enqueues no outbound command, so undo is durable on this Mac and invisible to the server forever. Undo is not out of scope by any reading -- MAC-01 is checked and names undo explicitly as a supported Mac operation, so it is one of the mutations this requirement asks to reconcile. O-45 is deliberately NOT the same work as O-41: POST /commands/undo-task takes a server-issued handle carried in the acknowledgement, which this client retains nowhere, so closing it means persisting and expiring undo handles alongside the mutation journal. Related but NOT blocking: O-43, a server-refused local change has no durable home and the next pull reverts it -- refused intent is not "accepted intent", so it falls outside this requirement’s clause and is a product decision. -->
...
| MAC-03 | Phase 3 | Blocked on O-45 — all eight command types reconcile against a real server, proven by arrival order and server read-back; undo alone enqueues nothing, and MAC-01 names undo |
