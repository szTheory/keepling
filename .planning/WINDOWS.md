...
| 95 | 06 | unrun-verify | .github/workflows/recovery-drills.yml |  | Class: scheduled-lane-never-bound-to-a-revision. The `daily-restore`, `weekly-historical-pitr` and `quarterly-host-replacement` lanes live in a schedule-triggered workflow, so no run of any of them exists AT the candidate release revision 26628e1 and all three are recorded as explicit zero-case BLOCKED entries in .planning/releases/candidate-1/release-manifest.json. They are not broken -- a scheduled drill passing last night is real evidence about last night's tree. What is missing is a way to bind a drill result to the exact revision being released, which is what every other lane in the manifest does. Their inventory owner was moved from KPL-06 to backlog because KPL-06 is ending. TO CLOSE: add `workflow_dispatch` to recovery-drills.yml so the drills can be run at a named revision, dispatch them at the release revision, and bind their run ids into that revision's manifest. STILL TRUE AT CANDIDATE-2 (2026-09-12): all three scheduled lanes are recorded as explicit zero-case BLOCKED entries in `.planning/releases/candidate-2/release-manifest.json` as well, at revision 23d79e5. Nothing in this candidate changes their binding problem; `workflow_dispatch` has still not been added. CORRECTED 2026-09-13. THIS ROW CONTAINED A FALSE STATEMENT: it said `workflow_dispatch` "has still not been added". It has been present since commit 231a1a5 (KPL-02-10), long before either release candidate, and `.github/workflows/recovery-drills.yml` carries a `drill` choice input covering all four fixtures. The binding problem this row exists to record is REAL and UNCHANGED -- no drill has been dispatched at a release revision and bound into a manifest -- but the stated reason was wrong, and the closing command should read: dispatch each drill at the release revision and bind its run id into that revision's manifest. WHAT DISPATCHING WOULD ACTUALLY PRODUCE TODAY, measured rather than assumed: every scheduled run fails, and by design. Runs 34565928148 (2026-09-11), 34675516855 (2026-09-12), 34740218044 and 34741243107 (2026-09-13) all end in failure at the credentials guard, which writes `result:"NON_PASSING", reason:"missing_protected_credentials"` and then exits 1 because none of the four KEEPLING_BACKUP_PRIMARY_* secrets is set. That is the drill declaring a loud non-pass rather than a silent skip, and it is NOT being changed here: silencing it would make the workflow green while proving nothing about recovery. The consequence to be aware of is that `Protected recovery drills` is a permanently red workflow until those secrets exist, so its redness carries no information. TO CLOSE, BOTH HALVES ARE NOW NAMED: provision the backup object-storage credentials so a drill can actually restore something, then dispatch at a release revision and bind the run id. CREDENTIALS HALF PROVISIONED 2026-09-13, BINDING HALF STILL OPEN. The four KEEPLING_BACKUP_PRIMARY_* secrets now exist in the `recovery-protected` environment and HCLOUD_TOKEN / CLOUDFLARE_API_TOKEN in `recovery-live`, which had to be created because it did not exist at all; `tooling/provision-release-secrets.sh` moves them from 1Password in one command and is reproducible on rotation. `Protected recovery drills` is consequently no longer permanently red: run 34776879634 passed both daily fixtures at revision 6c19cfe. THE CLOSING CONDITION OF THIS ROW IS UNCHANGED AND UNMET -- no drill has yet been dispatched at a release revision and bound into that revision's manifest. READ THIS ROW ONLY ALONGSIDE ROW 100: the restore the credentials now gate does not itself read them, so a passing drill is not evidence about the credentials. CLOSED 2026-09-14 at revision 4f59541. BOTH HALVES ARE NOW MET. Every lane in the schedule-triggered workflow is dispatched at this release revision and bound into that revision's manifest: daily-restore (run 34873301081, both fixtures), weekly-historical-pitr (run 34873365684) and quarterly-host-replacement (run 34871926793). Each drill's retained evidence names 4f59541 in its own `revision` field, written by the drill job itself, and the run ids are in .planning/releases/candidate-5/release-manifest.json under revision.runIds. candidate-4 had already bound three of the four at 069df06; what completes the row is the fourth. THE FOURTH ONE ALSO CORRECTS A MISREADING THIS ROW HELPED CARRY. quarterly-host-replacement was reported BLOCKED in candidates 1 through 4 on missing credentials, and candidate-4's manifest named an undecided DNS record name and SSH public key. Those inputs belong to `live-host-dns-acceptance`, a DIFFERENT lane with requiresSecrets TRUE. quarterly-host-replacement declares requiresSecrets FALSE and its command is ./tooling/test-host-replacement-sequence.sh, which needs no credential, no provider and no network. It was blocked because the job exited before running its own declared command -- window 83's failure mode, the fourth lane in this project found in it. WHAT IS NOT CLOSED BY THIS ROW: `live-host-dns-acceptance` stays BLOCKED, and DATA-03 and OPS-02 stay unchecked. That lane is sealed in code -- `credentialed_apply` ends in an unconditional die -- and is owned by BACKLOG/999.2 by its own disclosure. The quarterly-host-replacement job still exits 1 on its second step so that no skip is ever counted as evidence, and its whole red job log is committed unedited. READ THIS ROW ONLY ALONGSIDE ROW 100: the restore the credentials gate does not itself read them, so a passing drill is still not evidence about the credentials or about any real object store. | fixed |  | 2026-09-12T04:30:00.000Z | 2026-09-14T17:30:00.000Z | BACKLOG |
| 96 | 06 | unmet-truth | tooling/verify-release.mjs |  | Class: real-control-not-machine-checked. A SLSA Build Level 2 provenance attestation now exists for the release archive and it genuinely verifies -- `gh attestation verify Keepling.ditto.zip --repo szTheory/keepling` exits 0 against the artefact downloaded from run 34735177060, whose bytes re-hash to the attested subject digest 57f594657752182b21ee72b3b15a199af7481719ef64993fb4af3b187eb1704a. But `tooling/verify-release.mjs` CANNOT reproduce that check, and prints `attestation=UNVERIFIED` for all five artefact entries in `.planning/releases/candidate-2/release-manifest.json`, online and offline. The cause is structural, not a bug: `verifyAttestation` passes each entry's `artifactPath` to `gh attestation verify`, and those paths are committed RECORDS of the released bytes (a package manifest, three absence records, a source-tree record), while the attestation is over the 129 MB archive, which is deliberately not committed. So the strongest supply-chain claim this project now holds is the one claim its own release verifier cannot check, and a reader running the verifier sees a line that looks exactly like the line candidate-1 produced when no attestation existed at all. That ambiguity is recorded in the manifest's own header rather than left to be discovered. TO CLOSE: teach `verify-release.mjs` to verify BY DIGEST rather than by path -- `gh api /repos/szTheory/keepling/attestations/sha256:<archiveDigestSha256>`, which is exactly how this candidate's retained statement at `.planning/releases/candidate-2/evidence/attestation/slsa-provenance.json` was obtained -- and assert that the returned statement's subject digest, `predicateType`, builder id and resolved source gitCommit all match the manifest, degrading loudly to UNVERIFIED on a network failure exactly as it does today. OR attach the archive as a release asset (row 90) and verify the asset. | open |  | 2026-09-12T23:59:00.000Z |  | BACKLOG |
| 97 | 06 | unmet-truth | tooling/generate-sbom.mjs |  | Class: evidence-environment-sensitivity. The Hex CycloneDX document is NOT environment-independent, so "generated from source at the release revision" is true of the CI document and only approximately true of a local one. Measured 2026-09-12 by generating both at revision 23d79e5 and diffing component name@version: continuous integration produced 54 components, a local run produced 58, and the difference is one-directional -- the local run is a strict superset. The four extras are hex@2.5.1, inets@9.7.1, xmerl@2.2 and syntax_tools@undefined. None is a project dependency; they are the generator's own toolchain leaking out of the developer machine's dep tree, and one of them carries a literal undefined version, which is a malformed component entry rather than a real inventory line. NOTHING PUBLISHED IS WRONG: candidate-2 retains the CI document, which is the clean one, and no shipped inventory over-reports. What is wrong is that the generator's output depends on where it runs, so a maintainer checking an SBOM locally will not reproduce the released one and may reasonably conclude the released one is missing entries. TO CLOSE: have tooling/generate-sbom.mjs derive the Hex component set from mix.lock alone, and reject any component whose version is undefined rather than emitting it. DO NOT close this by pinning the expected count -- the defect is the environment dependence, not the number. OWNER: BACKLOG. | open |  | 2026-09-13T03:55:00.000Z |  | BACKLOG |
| 98 | 06 | unmet-truth | apps/server/test/keepling_web/compatibility_controller_test.exs | 146 | Class: clock-dependent-fixture. FOUND AND FIXED 2026-09-13, recorded because it was live in the tree for eleven days and nothing would have caught it before it fired. The compatibility suite wrote its support window as fixed dates -- `previous_superseded_at` 2026-06-15T12:00:00Z, `deprecation_deadline` 2026-09-13T12:00:00Z -- which is exactly the 90-day minimum `Keepling.Application.Compatibility.validate_distributed!/2` enforces, so the pair read as a deliberate boundary fixture rather than as a hazard. It was a time bomb: `KeeplingWeb.CompatibilityController` OVERWRITES the policy's `now` with `DateTime.utc_now/0` before answering, so the fixture's own `now` never reaches the decision. At 2026-09-13T12:00:00Z the previous protocol train stopped being supported, the advertised minimum rose from 1 to 2, and `test anonymous clients receive complete authoritative compatibility metadata` began failing on every run. MEASURED, NOT INFERRED: continuous integration last ran this suite at 2026-09-13T03:58Z and passed; the local reproduction at 12:50Z failed with left `minimum => 2` against right `minimum => 1`, and the same failure reproduces at clean HEAD f58d234, so it is not a consequence of the hackney dependency raise made the same day. It was invisible until now for a second reason worth keeping: the lane was already BLOCKED by row 85, and `run_lane` withholds a failing lane's output when the privacy scan does not pass, so this failure never reached any log a human read. FIXED by deriving both dates from ONE reading of the run's own clock (superseded = now - 1 day, deadline = superseded + 90 days), which preserves the exact boundary the validation cares about while keeping the deadline ahead of whenever the suite runs; one reading rather than two because two can straddle a second boundary and put the deadline below the minimum. `--lane server` now reports `status=PASS cases=337`. RESIDUAL RISK NOT CLOSED BY THIS ROW: no check exists that would catch the next clock-dependent fixture. A dated literal in a test is only distinguishable from a boundary fixture by reading the code that consumes it. | fixed |  | 2026-09-13T14:30:00.000Z | 2026-09-13T14:30:00.000Z | BACKLOG |
| 99 | 06 | unmet-truth | tooling/test-phase-2.sh | 93 | Class: diagnostic-surface-too-narrow. FOUND AND FIXED 2026-09-13, recorded because it is the reason row 86 stayed open for as long as it did and the reason row 85's cause was never visible. `run_lane` printed `sed -n '1,200p'` of a failing lane's log. For `image-compose-deploy`, whose log is dominated by Docker build output, the actual cause was past line 500 at every revision, so every continuous-integration run displayed 200 lines of `Getting <package> (Hex package)` and stopped -- which is why this lane was reported for weeks as a hackney advisory when five further blockers stood behind it. The cause of a failure is almost always near the end. Short logs now print in full; longer ones print the first 100 lines and the last 200 with an explicit count of what was dropped. THE SAME BLINDNESS EXISTED ONE LEVEL DOWN: a Compose service failing reports only `didn't complete successfully: exit 1`, and `compose down` then destroys the only copy of why, so `tooling/verify-compose.sh` and `tooling/verify-deploy.sh` now dump every service's log before teardown, on failure only. Both changes paid for themselves within one run each: the first produced `Image verification failed: PostgreSQL did not become ready`, the second produced `cat: /run/secrets/database_url: Permission denied`. RELATED BUT NOT FIXED HERE: `run_lane` withholds a failing lane's output ENTIRELY when the privacy scan does not pass, which is correct -- an unscanned log must not reach a public log -- but it means a lane can fail for a reason nobody may read. Rows 85 and 98 were both invisible for exactly that reason. A better design would redact the sentinels and print the rest rather than withholding everything, and that is NOT implemented. OWNER: BACKLOG. | open |  | 2026-09-13T14:30:00.000Z |  | BACKLOG |
| 100 | 06 | unmet-truth | .github/workflows/recovery-drills.yml | 37 | Class: gate-does-not-consume-what-it-guards. FOUND 2026-09-13 immediately after provisioning the backup credentials row 95 asks for, and recorded because the provisioning LOOKS like it proved more than it did. The `daily-restore` job declares KEEPLING_BACKUP_PRIMARY_ENDPOINT, _REGION, _ACCESS_KEY and _SECRET_KEY, and its guard step writes `result:"NON_PASSING", reason:"missing_protected_credentials"` and exits 1 when any is absent. But `tooling/verify-restore.sh` NEVER READS ANY OF THEM. It restores from `packages/contracts/vectors/recovery.json`, a committed fixture, with a disposable local target. The credentials are therefore a GATE, not an INPUT: the drill would pass identically with WRONG credentials, because no code path ever authenticates them. MEASURED, NOT INFERRED: run 34776879634 at revision 6c19cfe dispatched both daily fixtures and both PASSED -- newest-logical elapsed_seconds=49, latest-wal elapsed_seconds=103, both `semantic:"complete"` -- where the immediately prior scheduled run 34740218044 produced `NON_PASSING / missing_protected_credentials` for both. So the credentials did change the outcome, and the drills were genuinely failing on absent input rather than on anything they measured; that part of row 95 was correct. WHAT THE PASS DOES PROVE, and it is real: the restore and verification logic runs end to end on a clean hosted runner, rejects all five corrupt fixtures (missing_wal_segment, wrong_key, corrupt_manifest, stale_epoch, semantic_smoke_incomplete), rotates the epoch and reaches a semantically complete target. WHAT IT DOES NOT PROVE, and must never be cited as proving: that any byte was fetched from object storage, that the primary bucket contains a restorable backup, that the stored credentials are valid, or anything whatsoever about real RPO. THE ONLY EVIDENCE THE CREDENTIALS ARE VALID COMES FROM ELSEWHERE and is narrow: `tooling/provision-release-secrets.sh` derives the S3 endpoint and region by calling b2_authorize_account, which accepted the application key during --check. That authenticates the key. It does not inspect a single backup object. TO CLOSE: have the daily drill restore from the actual primary object store using the credentials it already demands, so that a wrong or revoked credential fails the drill. DO NOT CLOSE by removing the guard -- the guard is correct and is what makes the gap visible. OWNER: BACKLOG. | open |  | 2026-09-13T19:30:00.000Z |  | BACKLOG |
| 101 | 06 | unmet-truth | apps/desktop/test/e2e/guarded-navigation.spec.ts | 78 | Class: retry-masks-a-flake. FOUND 2026-09-13 while assembling release candidate-3, by reading the desktop-e2e lane's own Playwright summary rather than its exit status. The job SUCCEEDED and the workflow is green, so nothing in continuous integration draws attention to this. The summary says `Running 66 tests using 1 worker`, then `3 flaky`, then `63 passed (2.0m)`: 63 tests passed on the first attempt, 3 passed ONLY ON RETRY, and 66 passed in total. THE THREE, named so they can be chased: apps/desktop/test/e2e/guarded-navigation.spec.ts:78 (dirty editor: Cmd-N opens the discard dialog instead of navigating), apps/desktop/test/e2e/guarded-navigation.spec.ts:135 (keyboard-initiated Discard Changes discards and completes navigation, matching the mouse-initiated outcome), and apps/desktop/test/e2e/lifecycle.spec.ts:214 (D-20: a second launch against the SAME profile activates the resident instance instead of opening a duplicate window). MEASURED, NOT INFERRED: run 34777805267 at revision c2e4ce3. THIS IS A REGRESSION IN STABILITY, not a pre-existing condition: candidate-2's retained desktop-e2e log at revision 23d79e5 reports `66 passed (1.7m)` and carries NO flaky line at all, so all 66 passed first time there. The same 66 tests now need 3 retries and the suite takes 2.0m instead of 1.7m. TWO OF THE THREE ARE THE SAME FILE AND THE SAME FEATURE -- the guarded-navigation discard dialog -- which is a shape worth noting: a shared wait or a dialog-timing assumption is a more likely single cause than three independent flakes. WHY THIS IS A LEDGER ROW AND NOT A FOOTNOTE: a retried pass and a first-attempt pass are recorded identically by the exit status, so a lane that degrades from 0 to 3 retries looks exactly as green as one that does not, and the degradation is invisible until someone reads the summary. That is the same failure shape as row 99 (swallowed diagnostics) applied to a test runner instead of to a script. It is also exactly what QUAL-02's flake accountability clause exists to surface, which is why candidate-3's desktop-e2e entry records cases=66 WITH the retry count rather than the flattering 66 or the understated 63. RELATED AND ALREADY FIXED, do not confuse them: commit c2e4ce3 fixed a DIFFERENT flake in desktop-units, whose previous fix widened an assertion past the framework timeout that gated it and so could never have worked. TO CLOSE: reproduce the three locally with repeat runs, find whether the two guarded-navigation failures share a cause, and fix the wait rather than raising the retry count. DO NOT CLOSE by disabling retries -- retries are not the defect, the silence about them is. OWNER: BACKLOG. MEASURED TWICE MORE THE SAME DAY, AND THE FIRST FRAMING ABOVE IS CORRECTED BY IT. The e2e suite and everything it drives are BYTE-IDENTICAL across c2e4ce3, 12dbe9b and 9a0b062 -- the two later commits change only files under .planning/ -- so the three runs are three samples of one program, which is as close to a controlled repeat as continuous integration gets. Results: c2e4ce3 (run 34777805267) 3 flaky; 12dbe9b (run 34779465024) 1 flaky, guarded-navigation.spec.ts:135 alone; 9a0b062 (run 34779549131) 0 flaky, `66 passed (1.9m)` with no flaky line at all. WHAT THIS CORRECTS: the row above, and the desktop-e2e note in .planning/releases/candidate-3/release-manifest.json, both call this a REGRESSION IN STABILITY against candidate-2. That claim was one clean sample against one flaky sample, and it is STRONGER THAN THE EVIDENCE SUPPORTS. A run with 0 flaky at 9a0b062 shows the suite still reaches a clean pass on the same code, so what is established is NONDETERMINISM, not a step change in it; candidate-2's single clean run is equally one sample and cannot carry the comparison either. The manifest is left unedited on purpose -- it is the record of what was measured at ITS revision, and its numbers (66 passing, 3 on retry) are correct for that run. This row is where the correction lives, and the manifest's note points here. WHAT THE REPEAT STRENGTHENS RATHER THAN WEAKENS: guarded-navigation.spec.ts:135 appears in 2 of the 3 runs and is the only test to repeat, which makes it the highest-value single target and makes the shared-cause hypothesis for the two guarded-navigation cases more attractive, not less. THE UNDERLYING DEFECT IS UNCHANGED AND THE ROW STAYS OPEN: a retried pass and a first-attempt pass are still recorded identically by the exit status, all three of these runs are green, and only the summary distinguishes them. TO CLOSE, unchanged: find whether the two guarded-navigation failures share a cause and fix the wait. The sampling above is also the method -- repeat runs over unchanged code, counting retries, not reading exit statuses. FOURTH SAMPLE 2026-09-13, at 069df06 (run 34780345200), still byte-identical e2e code: 1 flaky, and it is guarded-navigation.spec.ts:135 AGAIN. The full series over four runs of one program is now 3, 1, 0, 1, and guarded-navigation.spec.ts:135 appears in THREE of the four -- the only test to recur at all, where guarded-navigation.spec.ts:78 and lifecycle.spec.ts:214 have appeared once each. THAT NARROWS THIS ROW FROM A SUITE-LEVEL CONCERN TO ONE TEST. The correction in the previous amendment stands unchanged: this is nondeterminism, not a regression, and all four runs are GREEN. What the fourth sample adds is a target cheap enough to act on -- one test, failing often enough (3 in 4) to reproduce locally with repeat runs rather than needing a long soak. REVISED TO CLOSE, more specific than before: run guarded-navigation.spec.ts:135 under `--repeat-each` locally until it fails, fix the wait it depends on, then re-measure the series. If fixing it also clears guarded-navigation.spec.ts:78 that confirms the shared-cause hypothesis; if it does not, the two are independent and 78 needs its own pass. The lane is recorded PASSED with cases=66 and the retry count in .planning/releases/candidate-4/release-manifest.json, the same way candidate-3 records it, so the series stays legible across manifests. FIFTH SAMPLE 2026-09-14, at 4f59541 (run 34868832072), still byte-identical e2e code: 0 flaky, 66 passed. The series over five runs of one program is now 3, 1, 0, 1, 0. TWO OF FIVE SAMPLES ARE CLEAN, and guarded-navigation.spec.ts:135 remains the only test to recur (3 of 5). THIS IS NOT EVIDENCE THE FLAKE IS FIXED, and the row stays OPEN: nothing in the e2e suite changed between these runs, so a clean sample is one more draw from the same nondeterministic distribution -- the mirror image of the mistake this row already corrects, where one flaky sample at c2e4ce3 was called a regression. A clean run is exactly as weak a signal in this direction as that one was in the other. The closing method is unchanged and still specific: run guarded-navigation.spec.ts:135 under `--repeat-each` locally until it fails, fix the wait it depends on, then re-measure. The lane is recorded PASSED with cases=66 and the retry count in .planning/releases/candidate-5/release-manifest.json, so the series stays legible across manifests. CLOSED 2026-09-14 -- CAUSE FOUND AND FIXED, and the cause is a PRODUCT DEFECT, not a test defect. REPRODUCTION FIRST, because the row demanded it: the prescribed `--repeat-each` run does NOT reproduce on this machine. guarded-navigation.spec.ts:135 passed 25/25 alone, and :78 and :135 together passed 30/30 with all twelve cores saturated by busy loops to widen the window -- 55 local executions, zero failures. That is a REAL RESULT and it is recorded rather than retried away: the closing method this row prescribed was not sufficient on its own, and anyone repeating it should expect it to come back clean. WHAT ACTUALLY LOCATED THE BUG was reading the retained CI failure text instead of re-running the test. Both flaky cases fail at the SAME assertion -- `await expect(dialog(window)).toBeVisible()` immediately after a keypress -- with `Error: element(s) not found` after the full 10s timeout. The dialog is never opened AT ALL, which has exactly one possible cause: the guard read a CLEAN editor and navigated. THE SHARED-CAUSE HYPOTHESIS THIS ROW CARRIED FROM ITS FIRST ENTRY IS CONFIRMED, and it is one bug, not two. THE MECHANISM: `Workspace`'s dirty-state guard read a `dirty` value that reaches it TWO ASYNC HOPS after the editor knows it. `TaskEditor` computes `dirty` during render and reports it from a PASSIVE effect (`onDirtyChange`); that calls `setDirty` in `Workspace`; only the resulting re-render rebuilds the `WorkspaceHandle` whose `guardedSetRoute` closes over the value. A `new-task`/`go-inbox`/`go-today` command delivered from the main process inside that gap reads a stale `false`, skips the dialog, and navigates away from an edit the person never saved. THIS IS SILENT DATA LOSS IN THE SHIPPED APP, reachable by any person who types and then hits Cmd-N/Cmd-1/Cmd-2 within a frame -- the Playwright cases were not flaky tests, they were the only thing in the repository fast enough to hit it reliably. The e2e suite was RIGHT and the retry was hiding a real defect, which is the strongest possible vindication of this row's founding claim that a retried pass and a first-attempt pass must not be recorded identically. PROVEN DETERMINISTICALLY, NOT BY TIMING: `packages/web-ui/src/workspace/Workspace.test.tsx` now dispatches the input event outside `act`, so React flushes the discrete update (render plus the commit phase where `useImperativeHandle` runs) while the passive effect that reports dirtiness upward is still queued, and delivers `guardedSetRoute` in that exact gap. Before the fix it fails with the identical symptom -- alertdialog not found, and the rendered DOM shows the route already moved to Today with the editor unmounted. There is no sleep, no retry and no timing dependency in it. THE FIX: `TaskEditorHandle` gains `isDirty()`, rebuilt during the commit phase of the render that computes `dirty` and so strictly earlier than either hop, and `attemptNavigation` consults it (`editorRef.current?.isDirty() ?? dirty`, falling back to the mirrored state so the guard fails CLOSED when no editor is mounted). The `dirty` state stays for rendering. THE RETRY COUNT WAS NOT TOUCHED and no assertion was weakened, as this row required. ALSO FIXED, the reason this took five CI runs to diagnose: the desktop-e2e job retained a Playwright trace and video for every failed attempt and then DISCARDED them, because a flake leaves the job GREEN and any `if: failure()` upload would never fire. `.github/workflows/desktop.yml` now uploads `apps/desktop/test-results/playwright` under `if: always()`, so the next occurrence of anything like this arrives with its own evidence attached. That is the same swallowed-diagnostics shape as row 99 and as this row's own founding complaint, one layer further out. VERIFIED: 306 desktop unit tests, 15 web-ui unit tests, both typechecks, and the full guarded-navigation e2e file repeated 6x (42 executions) all pass on the fixed bytes. WHAT IS NOT CLAIMED: that the CI interleaving was byte-for-byte the one the deterministic test constructs. It cannot be, from a log that retained no trace. What IS established is a demonstrated, sufficient mechanism that produces the identical observed symptom in both failing cases, and which is a genuine defect worth fixing on its own merits whether or not it was the only path to that symptom. If a guarded-navigation flake recurs after this, the newly uploaded trace is what should settle it -- and this paragraph is why that upload was added in the same commit. | fixed |  | 2026-09-13T20:45:00.000Z | 2026-09-14T21:30:00.000Z | BACKLOG |
| 102 | 06 | unmet-truth | tooling/release-lanes.json |  | Class: inventory-understates-blocker. FOUND 2026-09-14 while trying to wire the last two lanes that looked cheap after cross-adapter-phase went green. TWO ENTRIES IN THE CLOSED LANE INVENTORY DECLARE A COMMAND BROADER THAN THE LANE THEY NAME, and in both cases the extra scope is a lane that ANOTHER inventory entry already owns, with an honest blocker of its own. mcp-phase-non-model -- the name says non-model -- declared `node tooling/verify-mcp-phase.mjs`, the WHOLE gate, which runs the representative-model lane that mcp-representative-model owns (requiresSecrets=true, a paid model credential). ios-simulator -- the name says simulator -- declared `node tooling/verify-ios-phase.mjs && node tooling/verify-ios-phase.mjs --lane server-driven-sim`, and the first half is the WHOLE gate, which runs the device and server-driven-device lanes that ios-device owns (a physically present, unlocked iPhone). MEASURED, NOT INFERRED: both commands were run locally. The iOS gate reported LANE name=device status=FAIL cases=0 and LANE name=server-driven-device status=FAIL cases=0 alongside five PASSing simulator lanes; the MCP gate reported LANE name=representative-model status=BLOCKED cases=0 with the message that ANTHROPIC_API_KEY is not set, alongside 237 passing cases across its other five lanes. WHY THIS IS A LEDGER ROW AND NOT A ONE-LINE EDIT. Both entries recorded physicalBlocker=null / requiresSecrets=false and a wiringBlocker saying only that `no committed workflow job invokes it today`. Read together that says the sole missing ingredient is a job -- which is exactly what I told the owner at the end of the previous session, in those words, and it was WRONG IN THE EXPENSIVE DIRECTION: as declared, neither command could exit zero on any runner, hosted or local, so wiring a job would have produced a permanently red lane rather than a newly green one. The recorded metadata was true of each lane's SCOPE and false of its COMMAND, and nothing mechanical connected the two. That is the mirror image of the vacuity this phase has been removing everywhere else: not a check that passes without observing anything, but an inventory that makes a blocked lane look unblocked. FIXED, in the narrow way. Both runners gained `--exclude-lane <name>`, and the commands now name the lanes they actually mean. The flag is deliberately NOT free-form, because an exclusion that can name any lane is a loophole generator -- the cheapest way to turn a red gate green becomes excluding whatever failed. Each runner carries a CLOSED map from the gate lanes it will exclude to the tooling/release-lanes.json entry that owns that lane's evidence (representative-model -> mcp-representative-model; device and server-driven-device -> ios-device), the runner VERIFIES at run time that the owning entry exists in the committed inventory and refuses otherwise, an unknown lane name is a hard failure, and every excluded lane still prints an EXCLUDED line and is counted in the summary's new `excluded=` field so a narrowed run is legible AS narrowed. An exclusion therefore cannot remove evidence from the closed inventory; it can only leave it with the entry that already carried it. ALSO CORRECTED while in the file: the iOS runner's summary counted blocked lanes as failed (`failed=` used `!r.passed`), reporting failures that had not occurred. The MCP runner had already fixed exactly this and says so in a comment; the iOS runner had not. WHAT IS NOT CLAIMED, AND WHY THIS ROW IS OPEN RATHER THAN FIXED. Neither lane is wired and neither is recorded PASSED. The corrected mcp-phase-non-model command was run locally and its cross-adapter lane FAILED -- the electron leg timed out after 506s waiting for the Settings pane to report Connected -- where the same lane passed in 38s earlier the same day on the same code. That failure is NOT attributed to any change here: the dedicated cross-adapter-phase CI job runs the same four legs and passed at this exact revision, so the electron leg works. What it means is that the local result is order- or warmth-dependent and I do not yet know why, and a lane may not be recorded PASSED on a command whose only local run failed. TWO FURTHER LOCAL ATTEMPTS MADE THAT WORSE, NOT BETTER, and are recorded so nobody repeats them: re-running the cross-adapter lane alone failed 3 of its 4 legs (electron timeout, an iPhone Swift test, and an mcp out-of-band call refused as task_not_found), and a run with KEEPLING_E2E_POSTGRES_PORT overridden to this machine's spare port failed every lane in under a second because nothing was listening there -- that gate provisions its own database and must NOT be given that override. The variable that is unreliable here is this workstation, not the corrected command. TO CLOSE: wire both jobs, and produce one green run of each corrected command on CI -- which is the authority for both lanes anyway. The iOS subset additionally needs a job timeout above 50 minutes: its accessibility lane alone took 44 minutes locally. OWNER: BACKLOG. CLOSED 2026-09-15. Both conditions met at the SAME commit, 6003540. ios-simulator: CI run 34996420244 reports lanes=22 failed=0 blocked=0 excluded=2 across 463 executed cases, plus the command's second invocation (--lane server-driven-sim) PASS cases=4; the two EXCLUDED lines name device and server-driven-device with evidence_owned_by=ios-device, so the narrowed run is legible AS narrowed. mcp-phase-non-model: CI run 34996420275's job reports lanes=5 failed=0 blocked=0 excluded=1 across 237 cases (deterministic 194, adversarial 16, cross-adapter 16, simulated-client 9, protocol 2), with representative-model EXCLUDED to mcp-representative-model. The iOS job carries timeout-minutes: 150; its accessibility lane alone took 1782949 ms on the hosted runner, so the 50 this row warned against would indeed have killed it. GETTING HERE COST SEVEN COMMITS AND FIVE RED CI RUNS, AND NOT ONE DEFECT WAS IN THE CORRECTED COMMANDS. The first wired iOS run reported failed=21 of 22 lanes. What that number actually measured, in order of discovery: (1) three phase-gate runners killed their own children at Node's 1 MiB spawnSync maxBuffer default and reported the kill as a lane failure -- 15 of the 21; (2) the server-driven runner's -only-testing named the whole ServerDrivenTests CLASS, pulling in two cross-adapter RPC entry points that XCTFail without their KEEPLING_LANE_* parameters, so 4 passing scenarios reported as 6/2f; (3) two Swift tests hardcoded 13 contract vector files against a directory 05-11 and 06-08 had grown to 16, and one of them demanded ZERO wire-DTO-decodable objects where mcp-tools.json now carries 6 -- that assertion was a TRIPWIRE for an expired deviation, so it was replaced with the round-trip coverage D-14 asked for rather than having its threshold raised from 0 to 6; (4) UI-lane failures arrived with no assertion at all, a head-100/tail-200 excerpt dropping the .swift:NN: error: line into the elided middle of an XCUITest activity log; (5) a typed non-Latin title read back as 24 of 40 scalars on the hosted runner and verbatim locally -- a read outrunning typeText's slow path for scalars the hardware keyboard cannot produce; (6) the capture sheet's .dynamicType audit throwing an ENGINE-level error at 48 scalars, where 04-14 had measured and disclosed the same limitation for 512-scalar screens only. THE PATTERN IS THE ACTUAL FINDING: every one of those predates this window and not one is a regression. They accumulated in lanes nothing executed. The shape appeared a seventh time in the same session in an unrelated file -- --requirements in BOTH gates read the traceability table with .find(...), singular, against a table carrying one row per requirement, and so declared IOS-02..04 and MCP-02..05 absent from rows the table devotes one each to; no workflow job invokes --requirements, which is exactly why a broken id reader survived. An unwired check does not stay neutral. It rots silently and the whole bill comes due the day someone wires it. That deserves its own row rather than a clause in this one. NOT CLAIMED. The 463 simulator cases are the ios-simulator lane's evidence and nothing more. device and server-driven-device remain ios-device's, attested locally this session at 29 and 4 cases on a physical iPhone against a devicectl-installed signed build, and are excluded from this run BY NAME. representative-model remains BLOCKED on a paid credential and is excluded by name. The accessibility lane FAILED at 4bd7648 and 78fee8b and PASSED at a700ec9, 039ee8b, 3ad863a and 6003540 with an unchanged input_digest of bcb6a95277167611 -- identical inputs, different verdicts -- so its green here is one sample, not a proof of stability; four consecutive passes lower the estimated rate without retracting the observation. The capture sheet's .dynamicType narrowing is a disclosed engine limitation, not a fix, recorded in docs/testing/ios-testing.md. | fixed |  | 2026-09-14T22:40:00.000Z | 2026-09-15T00:00:00.000Z | BACKLOG |
...
[
  ...
  },
  {
    ...
    "resolved_at": "2026-09-12T02:03:21.000Z",
  ...
  },
  {
    ...
    "line": null,
    "description": "Class: physical-blocker. macOS integration row A14 (Light and Dark change while the app is open) cannot run on a hosted GitHub runner, and this is now an OBSERVED boundary rather than a prediction. A14 is the only row that changes a system appearance setting WHILE the application is already running and asserts the window re-themes without a relaunch; delivering that change to a running process needs a logged-in Aqua session whose appearance daemon posts AppleInterfaceThemeChangedNotification, which a hosted runner has not got. Measured both ways at revision 26628e1: `ROW id=A14 status=FAIL cases=4` on macos-15 with `background stayed` an all-zero colour (run 34668212473, job desktop-macos-integration), and `status=PASS cases=4` on a real logged-in session. Rows A10, A12 and A13 pass on the same runner because each applies its setting BEFORE launch. The row has been split out as the `macos-integration-live-appearance` lane, authority local-attested with a stated physical blocker, and emits an explicit zero-case BLOCKED entry into every release manifest rather than disappearing. It is NOT a product defect and must not be recorded as one. TO CLOSE with continuous-integration authority: run the row on a self-hosted macOS runner with a real logged-in session, against the exact continuous-integration-built bytes. ADVANCED 2026-09-12 (candidate-2): at revision 23d79e5 the `desktop-macos-integration` job no longer ATTEMPTS A14 at all -- the step was narrowed to A10, A12 and A13 and reports `rows=3 failed=0 cases=9`, so the job SUCCEEDS and `desktop-promote` was able to run for the first time (that is what closed row 88). THE ROW ITSELF IS NO CLOSER TO RUNNING: the split stopped a physically impossible row from failing a job; it did not give a hosted runner a logged-in Aqua session. `macos-integration-live-appearance` reports BLOCKED with a zero case count in `.planning/releases/candidate-2/release-manifest.json`, exactly as it did in candidate-1. The closing condition is unchanged.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-12T04:10:00.000Z",
    "resolved_at": null,
  ...
  },
  {
    ...
    "reason": "",
    "recorded_at": "2026-09-12T04:10:00.000Z",
    "resolved_at": "2026-09-13T14:30:00.000Z",
    "owner": "BACKLOG"
  },
  {
    "id": 86,
  ...
  },
  {
    ...
    "file": ".github/workflows/desktop.yml",
    "line": null,
    "description": "Class: no-continuous-integration-signing. Every continuous-integration-built desktop artefact is UNSIGNED and UNNOTARIZED, at every revision so far, because the Developer ID .p12 has not been exported to repository secrets. The `desktop-package` job detects this and takes its documented unsigned path, printing `No Developer ID certificate is reachable from this event (push). This artifact is UNSIGNED and UNNOTARIZED, and the package manifest records it as such`, and the manifest does record it (codeSigning.developerIdSigned false, notarization.status not-attempted) -- the machinery is behaving correctly and honestly. The consequence is what matters: a signed, notarized and stapled build HAS been produced and verified end to end locally (spctl --assess returns accepted with source Notarized Developer ID), but that attestation binds to LOCALLY BUILT bytes, and nothing local may bind to locally built bytes. So no signing claim may currently be attached to any released artefact, and `.planning/releases/candidate-1/release-manifest.json` states the unsigned fact directly on the artefact entry. TO CLOSE: export the Developer ID .p12 and its password to repository secrets (an owner action), re-run desktop.yml, and re-verify the manifest at the resulting revision. CLOSED 2026-09-12, at the revision that first built with the Developer ID secrets. The five Apple secrets were published from the single Developer ID Application identity (verified by SHA-1 fingerprint before upload, and confirmed against Apple's notary service for the team at publish time), and continuous integration then signed, notarized and stapled for the first time. Proof from the `desktop-package` job's own log, not from assertion: \"1 identity imported.\", KEEPLING_MACOS_SIGNING_IDENTITY resolving to the expected fingerprint 70D08D171695D0E6126B7C97325A16AB14A76945, \"Desktop package signing: Developer ID identity resolved\", and \"The staple and validate action worked!\". `desktop-packaged` and `desktop-macos-integration` both PASS against those signed bytes. NOTE FOR READERS OF THE RETAINED EVIDENCE: the candidate-1 release manifest is bound to revision 26628e1, which PREDATES this and is genuinely unsigned; this row closing does not retroactively make that candidate's artefacts signed.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-12T04:10:00.000Z",
    "resolved_at": "2026-09-12T21:40:00.000Z",
    "owner": "BACKLOG"
  },
  {
    "id": 92,
  ...
  },
  {
    ...
    "line": null,
    "description": "Class: scheduled-lane-never-bound-to-a-revision. The `daily-restore`, `weekly-historical-pitr` and `quarterly-host-replacement` lanes live in a schedule-triggered workflow, so no run of any of them exists AT the candidate release revision 26628e1 and all three are recorded as explicit zero-case BLOCKED entries in .planning/releases/candidate-1/release-manifest.json. They are not broken -- a scheduled drill passing last night is real evidence about last night's tree. What is missing is a way to bind a drill result to the exact revision being released, which is what every other lane in the manifest does. Their inventory owner was moved from KPL-06 to backlog because KPL-06 is ending. TO CLOSE: add `workflow_dispatch` to recovery-drills.yml so the drills can be run at a named revision, dispatch them at the release revision, and bind their run ids into that revision's manifest. STILL TRUE AT CANDIDATE-2 (2026-09-12): all three scheduled lanes are recorded as explicit zero-case BLOCKED entries in `.planning/releases/candidate-2/release-manifest.json` as well, at revision 23d79e5. Nothing in this candidate changes their binding problem; `workflow_dispatch` has still not been added. CORRECTED 2026-09-13. THIS ROW CONTAINED A FALSE STATEMENT: it said `workflow_dispatch` \"has still not been added\". It has been present since commit 231a1a5 (KPL-02-10), long before either release candidate, and `.github/workflows/recovery-drills.yml` carries a `drill` choice input covering all four fixtures. The binding problem this row exists to record is REAL and UNCHANGED -- no drill has been dispatched at a release revision and bound into a manifest -- but the stated reason was wrong, and the closing command should read: dispatch each drill at the release revision and bind its run id into that revision's manifest. WHAT DISPATCHING WOULD ACTUALLY PRODUCE TODAY, measured rather than assumed: every scheduled run fails, and by design. Runs 34565928148 (2026-09-11), 34675516855 (2026-09-12), 34740218044 and 34741243107 (2026-09-13) all end in failure at the credentials guard, which writes `result:\"NON_PASSING\", reason:\"missing_protected_credentials\"` and then exits 1 because none of the four KEEPLING_BACKUP_PRIMARY_* secrets is set. That is the drill declaring a loud non-pass rather than a silent skip, and it is NOT being changed here: silencing it would make the workflow green while proving nothing about recovery. The consequence to be aware of is that `Protected recovery drills` is a permanently red workflow until those secrets exist, so its redness carries no information. TO CLOSE, BOTH HALVES ARE NOW NAMED: provision the backup object-storage credentials so a drill can actually restore something, then dispatch at a release revision and bind the run id. CREDENTIALS HALF PROVISIONED 2026-09-13, BINDING HALF STILL OPEN. The four KEEPLING_BACKUP_PRIMARY_* secrets now exist in the `recovery-protected` environment and HCLOUD_TOKEN / CLOUDFLARE_API_TOKEN in `recovery-live`, which had to be created because it did not exist at all; `tooling/provision-release-secrets.sh` moves them from 1Password in one command and is reproducible on rotation. `Protected recovery drills` is consequently no longer permanently red: run 34776879634 passed both daily fixtures at revision 6c19cfe. THE CLOSING CONDITION OF THIS ROW IS UNCHANGED AND UNMET -- no drill has yet been dispatched at a release revision and bound into that revision's manifest. READ THIS ROW ONLY ALONGSIDE ROW 100: the restore the credentials now gate does not itself read them, so a passing drill is not evidence about the credentials. CLOSED 2026-09-14 at revision 4f59541. BOTH HALVES ARE NOW MET. Every lane in the schedule-triggered workflow is dispatched at this release revision and bound into that revision's manifest: daily-restore (run 34873301081, both fixtures), weekly-historical-pitr (run 34873365684) and quarterly-host-replacement (run 34871926793). Each drill's retained evidence names 4f59541 in its own `revision` field, written by the drill job itself, and the run ids are in .planning/releases/candidate-5/release-manifest.json under revision.runIds. candidate-4 had already bound three of the four at 069df06; what completes the row is the fourth. THE FOURTH ONE ALSO CORRECTS A MISREADING THIS ROW HELPED CARRY. quarterly-host-replacement was reported BLOCKED in candidates 1 through 4 on missing credentials, and candidate-4's manifest named an undecided DNS record name and SSH public key. Those inputs belong to `live-host-dns-acceptance`, a DIFFERENT lane with requiresSecrets TRUE. quarterly-host-replacement declares requiresSecrets FALSE and its command is ./tooling/test-host-replacement-sequence.sh, which needs no credential, no provider and no network. It was blocked because the job exited before running its own declared command -- window 83's failure mode, the fourth lane in this project found in it. WHAT IS NOT CLOSED BY THIS ROW: `live-host-dns-acceptance` stays BLOCKED, and DATA-03 and OPS-02 stay unchecked. That lane is sealed in code -- `credentialed_apply` ends in an unconditional die -- and is owned by BACKLOG/999.2 by its own disclosure. The quarterly-host-replacement job still exits 1 on its second step so that no skip is ever counted as evidence, and its whole red job log is committed unedited. READ THIS ROW ONLY ALONGSIDE ROW 100: the restore the credentials gate does not itself read them, so a passing drill is still not evidence about the credentials or about any real object store.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-12T04:30:00.000Z",
    "resolved_at": "2026-09-14T17:30:00.000Z",
  ...
  },
  {
    ...
    "kind": "unmet-truth",
    "phase": "06",
    "file": "tooling/verify-release.mjs",
    "line": null,
    "description": "Class: real-control-not-machine-checked. A SLSA Build Level 2 provenance attestation now exists for the release archive and it genuinely verifies -- `gh attestation verify Keepling.ditto.zip --repo szTheory/keepling` exits 0 against the artefact downloaded from run 34735177060, whose bytes re-hash to the attested subject digest 57f594657752182b21ee72b3b15a199af7481719ef64993fb4af3b187eb1704a. But `tooling/verify-release.mjs` CANNOT reproduce that check, and prints `attestation=UNVERIFIED` for all five artefact entries in `.planning/releases/candidate-2/release-manifest.json`, online and offline. The cause is structural, not a bug: `verifyAttestation` passes each entry's `artifactPath` to `gh attestation verify`, and those paths are committed RECORDS of the released bytes (a package manifest, three absence records, a source-tree record), while the attestation is over the 129 MB archive, which is deliberately not committed. So the strongest supply-chain claim this project now holds is the one claim its own release verifier cannot check, and a reader running the verifier sees a line that looks exactly like the line candidate-1 produced when no attestation existed at all. That ambiguity is recorded in the manifest's own header rather than left to be discovered. TO CLOSE: teach `verify-release.mjs` to verify BY DIGEST rather than by path -- `gh api /repos/szTheory/keepling/attestations/sha256:<archiveDigestSha256>`, which is exactly how this candidate's retained statement at `.planning/releases/candidate-2/evidence/attestation/slsa-provenance.json` was obtained -- and assert that the returned statement's subject digest, `predicateType`, builder id and resolved source gitCommit all match the manifest, degrading loudly to UNVERIFIED on a network failure exactly as it does today. OR attach the archive as a release asset (row 90) and verify the asset.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-12T23:59:00.000Z",
    "resolved_at": null,
    "owner": "BACKLOG"
  },
  {
    ...
    "kind": "unmet-truth",
    "phase": "06",
    "file": "tooling/generate-sbom.mjs",
    "line": null,
    "description": "Class: evidence-environment-sensitivity. The Hex CycloneDX document is NOT environment-independent, so \"generated from source at the release revision\" is true of the CI document and only approximately true of a local one. Measured 2026-09-12 by generating both at revision 23d79e5 and diffing component name@version: continuous integration produced 54 components, a local run produced 58, and the difference is one-directional -- the local run is a strict superset. The four extras are hex@2.5.1, inets@9.7.1, xmerl@2.2 and syntax_tools@undefined. None is a project dependency; they are the generator's own toolchain leaking out of the developer machine's dep tree, and one of them carries a literal undefined version, which is a malformed component entry rather than a real inventory line. NOTHING PUBLISHED IS WRONG: candidate-2 retains the CI document, which is the clean one, and no shipped inventory over-reports. What is wrong is that the generator's output depends on where it runs, so a maintainer checking an SBOM locally will not reproduce the released one and may reasonably conclude the released one is missing entries. TO CLOSE: have tooling/generate-sbom.mjs derive the Hex component set from mix.lock alone, and reject any component whose version is undefined rather than emitting it. DO NOT close this by pinning the expected count -- the defect is the environment dependence, not the number. OWNER: BACKLOG.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-13T03:55:00.000Z",
    "resolved_at": null,
    "owner": "BACKLOG"
  },
  {
    ...
    "kind": "unmet-truth",
    "phase": "06",
    "file": "apps/server/test/keepling_web/compatibility_controller_test.exs",
    "line": 146,
    "description": "Class: clock-dependent-fixture. FOUND AND FIXED 2026-09-13, recorded because it was live in the tree for eleven days and nothing would have caught it before it fired. The compatibility suite wrote its support window as fixed dates -- `previous_superseded_at` 2026-06-15T12:00:00Z, `deprecation_deadline` 2026-09-13T12:00:00Z -- which is exactly the 90-day minimum `Keepling.Application.Compatibility.validate_distributed!/2` enforces, so the pair read as a deliberate boundary fixture rather than as a hazard. It was a time bomb: `KeeplingWeb.CompatibilityController` OVERWRITES the policy's `now` with `DateTime.utc_now/0` before answering, so the fixture's own `now` never reaches the decision. At 2026-09-13T12:00:00Z the previous protocol train stopped being supported, the advertised minimum rose from 1 to 2, and `test anonymous clients receive complete authoritative compatibility metadata` began failing on every run. MEASURED, NOT INFERRED: continuous integration last ran this suite at 2026-09-13T03:58Z and passed; the local reproduction at 12:50Z failed with left `minimum => 2` against right `minimum => 1`, and the same failure reproduces at clean HEAD f58d234, so it is not a consequence of the hackney dependency raise made the same day. It was invisible until now for a second reason worth keeping: the lane was already BLOCKED by row 85, and `run_lane` withholds a failing lane's output when the privacy scan does not pass, so this failure never reached any log a human read. FIXED by deriving both dates from ONE reading of the run's own clock (superseded = now - 1 day, deadline = superseded + 90 days), which preserves the exact boundary the validation cares about while keeping the deadline ahead of whenever the suite runs; one reading rather than two because two can straddle a second boundary and put the deadline below the minimum. `--lane server` now reports `status=PASS cases=337`. RESIDUAL RISK NOT CLOSED BY THIS ROW: no check exists that would catch the next clock-dependent fixture. A dated literal in a test is only distinguishable from a boundary fixture by reading the code that consumes it.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-13T14:30:00.000Z",
    "resolved_at": "2026-09-13T14:30:00.000Z",
    "owner": "BACKLOG"
  },
  {
    ...
    "file": "tooling/test-phase-2.sh",
    "line": 93,
    "description": "Class: diagnostic-surface-too-narrow. FOUND AND FIXED 2026-09-13, recorded because it is the reason row 86 stayed open for as long as it did and the reason row 85's cause was never visible. `run_lane` printed `sed -n '1,200p'` of a failing lane's log. For `image-compose-deploy`, whose log is dominated by Docker build output, the actual cause was past line 500 at every revision, so every continuous-integration run displayed 200 lines of `Getting <package> (Hex package)` and stopped -- which is why this lane was reported for weeks as a hackney advisory when five further blockers stood behind it. The cause of a failure is almost always near the end. Short logs now print in full; longer ones print the first 100 lines and the last 200 with an explicit count of what was dropped. THE SAME BLINDNESS EXISTED ONE LEVEL DOWN: a Compose service failing reports only `didn't complete successfully: exit 1`, and `compose down` then destroys the only copy of why, so `tooling/verify-compose.sh` and `tooling/verify-deploy.sh` now dump every service's log before teardown, on failure only. Both changes paid for themselves within one run each: the first produced `Image verification failed: PostgreSQL did not become ready`, the second produced `cat: /run/secrets/database_url: Permission denied`. RELATED BUT NOT FIXED HERE: `run_lane` withholds a failing lane's output ENTIRELY when the privacy scan does not pass, which is correct -- an unscanned log must not reach a public log -- but it means a lane can fail for a reason nobody may read. Rows 85 and 98 were both invisible for exactly that reason. A better design would redact the sentinels and print the rest rather than withholding everything, and that is NOT implemented. OWNER: BACKLOG.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-13T14:30:00.000Z",
    "resolved_at": null,
  ...
  },
  {
    ...
    "file": ".github/workflows/recovery-drills.yml",
    "line": 37,
    "description": "Class: gate-does-not-consume-what-it-guards. FOUND 2026-09-13 immediately after provisioning the backup credentials row 95 asks for, and recorded because the provisioning LOOKS like it proved more than it did. The `daily-restore` job declares KEEPLING_BACKUP_PRIMARY_ENDPOINT, _REGION, _ACCESS_KEY and _SECRET_KEY, and its guard step writes `result:\"NON_PASSING\", reason:\"missing_protected_credentials\"` and exits 1 when any is absent. But `tooling/verify-restore.sh` NEVER READS ANY OF THEM. It restores from `packages/contracts/vectors/recovery.json`, a committed fixture, with a disposable local target. The credentials are therefore a GATE, not an INPUT: the drill would pass identically with WRONG credentials, because no code path ever authenticates them. MEASURED, NOT INFERRED: run 34776879634 at revision 6c19cfe dispatched both daily fixtures and both PASSED -- newest-logical elapsed_seconds=49, latest-wal elapsed_seconds=103, both `semantic:\"complete\"` -- where the immediately prior scheduled run 34740218044 produced `NON_PASSING / missing_protected_credentials` for both. So the credentials did change the outcome, and the drills were genuinely failing on absent input rather than on anything they measured; that part of row 95 was correct. WHAT THE PASS DOES PROVE, and it is real: the restore and verification logic runs end to end on a clean hosted runner, rejects all five corrupt fixtures (missing_wal_segment, wrong_key, corrupt_manifest, stale_epoch, semantic_smoke_incomplete), rotates the epoch and reaches a semantically complete target. WHAT IT DOES NOT PROVE, and must never be cited as proving: that any byte was fetched from object storage, that the primary bucket contains a restorable backup, that the stored credentials are valid, or anything whatsoever about real RPO. THE ONLY EVIDENCE THE CREDENTIALS ARE VALID COMES FROM ELSEWHERE and is narrow: `tooling/provision-release-secrets.sh` derives the S3 endpoint and region by calling b2_authorize_account, which accepted the application key during --check. That authenticates the key. It does not inspect a single backup object. TO CLOSE: have the daily drill restore from the actual primary object store using the credentials it already demands, so that a wrong or revoked credential fails the drill. DO NOT CLOSE by removing the guard -- the guard is correct and is what makes the gap visible. OWNER: BACKLOG.",
    "status": "open",
    "reason": "",
    "recorded_at": "2026-09-13T19:30:00.000Z",
    "resolved_at": null,
    "owner": "BACKLOG"
  },
  {
    ...
    "kind": "unmet-truth",
    "phase": "06",
    "file": "apps/desktop/test/e2e/guarded-navigation.spec.ts",
    "line": 78,
    "description": "Class: retry-masks-a-flake. FOUND 2026-09-13 while assembling release candidate-3, by reading the desktop-e2e lane's own Playwright summary rather than its exit status. The job SUCCEEDED and the workflow is green, so nothing in continuous integration draws attention to this. The summary says `Running 66 tests using 1 worker`, then `3 flaky`, then `63 passed (2.0m)`: 63 tests passed on the first attempt, 3 passed ONLY ON RETRY, and 66 passed in total. THE THREE, named so they can be chased: apps/desktop/test/e2e/guarded-navigation.spec.ts:78 (dirty editor: Cmd-N opens the discard dialog instead of navigating), apps/desktop/test/e2e/guarded-navigation.spec.ts:135 (keyboard-initiated Discard Changes discards and completes navigation, matching the mouse-initiated outcome), and apps/desktop/test/e2e/lifecycle.spec.ts:214 (D-20: a second launch against the SAME profile activates the resident instance instead of opening a duplicate window). MEASURED, NOT INFERRED: run 34777805267 at revision c2e4ce3. THIS IS A REGRESSION IN STABILITY, not a pre-existing condition: candidate-2's retained desktop-e2e log at revision 23d79e5 reports `66 passed (1.7m)` and carries NO flaky line at all, so all 66 passed first time there. The same 66 tests now need 3 retries and the suite takes 2.0m instead of 1.7m. TWO OF THE THREE ARE THE SAME FILE AND THE SAME FEATURE -- the guarded-navigation discard dialog -- which is a shape worth noting: a shared wait or a dialog-timing assumption is a more likely single cause than three independent flakes. WHY THIS IS A LEDGER ROW AND NOT A FOOTNOTE: a retried pass and a first-attempt pass are recorded identically by the exit status, so a lane that degrades from 0 to 3 retries looks exactly as green as one that does not, and the degradation is invisible until someone reads the summary. That is the same failure shape as row 99 (swallowed diagnostics) applied to a test runner instead of to a script. It is also exactly what QUAL-02's flake accountability clause exists to surface, which is why candidate-3's desktop-e2e entry records cases=66 WITH the retry count rather than the flattering 66 or the understated 63. RELATED AND ALREADY FIXED, do not confuse them: commit c2e4ce3 fixed a DIFFERENT flake in desktop-units, whose previous fix widened an assertion past the framework timeout that gated it and so could never have worked. TO CLOSE: reproduce the three locally with repeat runs, find whether the two guarded-navigation failures share a cause, and fix the wait rather than raising the retry count. DO NOT CLOSE by disabling retries -- retries are not the defect, the silence about them is. OWNER: BACKLOG. MEASURED TWICE MORE THE SAME DAY, AND THE FIRST FRAMING ABOVE IS CORRECTED BY IT. The e2e suite and everything it drives are BYTE-IDENTICAL across c2e4ce3, 12dbe9b and 9a0b062 -- the two later commits change only files under .planning/ -- so the three runs are three samples of one program, which is as close to a controlled repeat as continuous integration gets. Results: c2e4ce3 (run 34777805267) 3 flaky; 12dbe9b (run 34779465024) 1 flaky, guarded-navigation.spec.ts:135 alone; 9a0b062 (run 34779549131) 0 flaky, `66 passed (1.9m)` with no flaky line at all. WHAT THIS CORRECTS: the row above, and the desktop-e2e note in .planning/releases/candidate-3/release-manifest.json, both call this a REGRESSION IN STABILITY against candidate-2. That claim was one clean sample against one flaky sample, and it is STRONGER THAN THE EVIDENCE SUPPORTS. A run with 0 flaky at 9a0b062 shows the suite still reaches a clean pass on the same code, so what is established is NONDETERMINISM, not a step change in it; candidate-2's single clean run is equally one sample and cannot carry the comparison either. The manifest is left unedited on purpose -- it is the record of what was measured at ITS revision, and its numbers (66 passing, 3 on retry) are correct for that run. This row is where the correction lives, and the manifest's note points here. WHAT THE REPEAT STRENGTHENS RATHER THAN WEAKENS: guarded-navigation.spec.ts:135 appears in 2 of the 3 runs and is the only test to repeat, which makes it the highest-value single target and makes the shared-cause hypothesis for the two guarded-navigation cases more attractive, not less. THE UNDERLYING DEFECT IS UNCHANGED AND THE ROW STAYS OPEN: a retried pass and a first-attempt pass are still recorded identically by the exit status, all three of these runs are green, and only the summary distinguishes them. TO CLOSE, unchanged: find whether the two guarded-navigation failures share a cause and fix the wait. The sampling above is also the method -- repeat runs over unchanged code, counting retries, not reading exit statuses. FOURTH SAMPLE 2026-09-13, at 069df06 (run 34780345200), still byte-identical e2e code: 1 flaky, and it is guarded-navigation.spec.ts:135 AGAIN. The full series over four runs of one program is now 3, 1, 0, 1, and guarded-navigation.spec.ts:135 appears in THREE of the four -- the only test to recur at all, where guarded-navigation.spec.ts:78 and lifecycle.spec.ts:214 have appeared once each. THAT NARROWS THIS ROW FROM A SUITE-LEVEL CONCERN TO ONE TEST. The correction in the previous amendment stands unchanged: this is nondeterminism, not a regression, and all four runs are GREEN. What the fourth sample adds is a target cheap enough to act on -- one test, failing often enough (3 in 4) to reproduce locally with repeat runs rather than needing a long soak. REVISED TO CLOSE, more specific than before: run guarded-navigation.spec.ts:135 under `--repeat-each` locally until it fails, fix the wait it depends on, then re-measure the series. If fixing it also clears guarded-navigation.spec.ts:78 that confirms the shared-cause hypothesis; if it does not, the two are independent and 78 needs its own pass. The lane is recorded PASSED with cases=66 and the retry count in .planning/releases/candidate-4/release-manifest.json, the same way candidate-3 records it, so the series stays legible across manifests. FIFTH SAMPLE 2026-09-14, at 4f59541 (run 34868832072), still byte-identical e2e code: 0 flaky, 66 passed. The series over five runs of one program is now 3, 1, 0, 1, 0. TWO OF FIVE SAMPLES ARE CLEAN, and guarded-navigation.spec.ts:135 remains the only test to recur (3 of 5). THIS IS NOT EVIDENCE THE FLAKE IS FIXED, and the row stays OPEN: nothing in the e2e suite changed between these runs, so a clean sample is one more draw from the same nondeterministic distribution -- the mirror image of the mistake this row already corrects, where one flaky sample at c2e4ce3 was called a regression. A clean run is exactly as weak a signal in this direction as that one was in the other. The closing method is unchanged and still specific: run guarded-navigation.spec.ts:135 under `--repeat-each` locally until it fails, fix the wait it depends on, then re-measure. The lane is recorded PASSED with cases=66 and the retry count in .planning/releases/candidate-5/release-manifest.json, so the series stays legible across manifests. CLOSED 2026-09-14 -- CAUSE FOUND AND FIXED, and the cause is a PRODUCT DEFECT, not a test defect. REPRODUCTION FIRST, because the row demanded it: the prescribed `--repeat-each` run does NOT reproduce on this machine. guarded-navigation.spec.ts:135 passed 25/25 alone, and :78 and :135 together passed 30/30 with all twelve cores saturated by busy loops to widen the window -- 55 local executions, zero failures. That is a REAL RESULT and it is recorded rather than retried away: the closing method this row prescribed was not sufficient on its own, and anyone repeating it should expect it to come back clean. WHAT ACTUALLY LOCATED THE BUG was reading the retained CI failure text instead of re-running the test. Both flaky cases fail at the SAME assertion -- `await expect(dialog(window)).toBeVisible()` immediately after a keypress -- with `Error: element(s) not found` after the full 10s timeout. The dialog is never opened AT ALL, which has exactly one possible cause: the guard read a CLEAN editor and navigated. THE SHARED-CAUSE HYPOTHESIS THIS ROW CARRIED FROM ITS FIRST ENTRY IS CONFIRMED, and it is one bug, not two. THE MECHANISM: `Workspace`'s dirty-state guard read a `dirty` value that reaches it TWO ASYNC HOPS after the editor knows it. `TaskEditor` computes `dirty` during render and reports it from a PASSIVE effect (`onDirtyChange`); that calls `setDirty` in `Workspace`; only the resulting re-render rebuilds the `WorkspaceHandle` whose `guardedSetRoute` closes over the value. A `new-task`/`go-inbox`/`go-today` command delivered from the main process inside that gap reads a stale `false`, skips the dialog, and navigates away from an edit the person never saved. THIS IS SILENT DATA LOSS IN THE SHIPPED APP, reachable by any person who types and then hits Cmd-N/Cmd-1/Cmd-2 within a frame -- the Playwright cases were not flaky tests, they were the only thing in the repository fast enough to hit it reliably. The e2e suite was RIGHT and the retry was hiding a real defect, which is the strongest possible vindication of this row's founding claim that a retried pass and a first-attempt pass must not be recorded identically. PROVEN DETERMINISTICALLY, NOT BY TIMING: `packages/web-ui/src/workspace/Workspace.test.tsx` now dispatches the input event outside `act`, so React flushes the discrete update (render plus the commit phase where `useImperativeHandle` runs) while the passive effect that reports dirtiness upward is still queued, and delivers `guardedSetRoute` in that exact gap. Before the fix it fails with the identical symptom -- alertdialog not found, and the rendered DOM shows the route already moved to Today with the editor unmounted. There is no sleep, no retry and no timing dependency in it. THE FIX: `TaskEditorHandle` gains `isDirty()`, rebuilt during the commit phase of the render that computes `dirty` and so strictly earlier than either hop, and `attemptNavigation` consults it (`editorRef.current?.isDirty() ?? dirty`, falling back to the mirrored state so the guard fails CLOSED when no editor is mounted). The `dirty` state stays for rendering. THE RETRY COUNT WAS NOT TOUCHED and no assertion was weakened, as this row required. ALSO FIXED, the reason this took five CI runs to diagnose: the desktop-e2e job retained a Playwright trace and video for every failed attempt and then DISCARDED them, because a flake leaves the job GREEN and any `if: failure()` upload would never fire. `.github/workflows/desktop.yml` now uploads `apps/desktop/test-results/playwright` under `if: always()`, so the next occurrence of anything like this arrives with its own evidence attached. That is the same swallowed-diagnostics shape as row 99 and as this row's own founding complaint, one layer further out. VERIFIED: 306 desktop unit tests, 15 web-ui unit tests, both typechecks, and the full guarded-navigation e2e file repeated 6x (42 executions) all pass on the fixed bytes. WHAT IS NOT CLAIMED: that the CI interleaving was byte-for-byte the one the deterministic test constructs. It cannot be, from a log that retained no trace. What IS established is a demonstrated, sufficient mechanism that produces the identical observed symptom in both failing cases, and which is a genuine defect worth fixing on its own merits whether or not it was the only path to that symptom. If a guarded-navigation flake recurs after this, the newly uploaded trace is what should settle it -- and this paragraph is why that upload was added in the same commit.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-13T20:45:00.000Z",
    "resolved_at": "2026-09-14T21:30:00.000Z",
    "owner": "BACKLOG"
  },
  {
    "id": 102,
    ...
    "file": "tooling/release-lanes.json",
    "line": null,
    "description": "Class: inventory-understates-blocker. FOUND 2026-09-14 while trying to wire the last two lanes that looked cheap after cross-adapter-phase went green. TWO ENTRIES IN THE CLOSED LANE INVENTORY DECLARE A COMMAND BROADER THAN THE LANE THEY NAME, and in both cases the extra scope is a lane that ANOTHER inventory entry already owns, with an honest blocker of its own. mcp-phase-non-model -- the name says non-model -- declared `node tooling/verify-mcp-phase.mjs`, the WHOLE gate, which runs the representative-model lane that mcp-representative-model owns (requiresSecrets=true, a paid model credential). ios-simulator -- the name says simulator -- declared `node tooling/verify-ios-phase.mjs && node tooling/verify-ios-phase.mjs --lane server-driven-sim`, and the first half is the WHOLE gate, which runs the device and server-driven-device lanes that ios-device owns (a physically present, unlocked iPhone). MEASURED, NOT INFERRED: both commands were run locally. The iOS gate reported LANE name=device status=FAIL cases=0 and LANE name=server-driven-device status=FAIL cases=0 alongside five PASSing simulator lanes; the MCP gate reported LANE name=representative-model status=BLOCKED cases=0 with the message that ANTHROPIC_API_KEY is not set, alongside 237 passing cases across its other five lanes. WHY THIS IS A LEDGER ROW AND NOT A ONE-LINE EDIT. Both entries recorded physicalBlocker=null / requiresSecrets=false and a wiringBlocker saying only that `no committed workflow job invokes it today`. Read together that says the sole missing ingredient is a job -- which is exactly what I told the owner at the end of the previous session, in those words, and it was WRONG IN THE EXPENSIVE DIRECTION: as declared, neither command could exit zero on any runner, hosted or local, so wiring a job would have produced a permanently red lane rather than a newly green one. The recorded metadata was true of each lane's SCOPE and false of its COMMAND, and nothing mechanical connected the two. That is the mirror image of the vacuity this phase has been removing everywhere else: not a check that passes without observing anything, but an inventory that makes a blocked lane look unblocked. FIXED, in the narrow way. Both runners gained `--exclude-lane <name>`, and the commands now name the lanes they actually mean. The flag is deliberately NOT free-form, because an exclusion that can name any lane is a loophole generator -- the cheapest way to turn a red gate green becomes excluding whatever failed. Each runner carries a CLOSED map from the gate lanes it will exclude to the tooling/release-lanes.json entry that owns that lane's evidence (representative-model -> mcp-representative-model; device and server-driven-device -> ios-device), the runner VERIFIES at run time that the owning entry exists in the committed inventory and refuses otherwise, an unknown lane name is a hard failure, and every excluded lane still prints an EXCLUDED line and is counted in the summary's new `excluded=` field so a narrowed run is legible AS narrowed. An exclusion therefore cannot remove evidence from the closed inventory; it can only leave it with the entry that already carried it. ALSO CORRECTED while in the file: the iOS runner's summary counted blocked lanes as failed (`failed=` used `!r.passed`), reporting failures that had not occurred. The MCP runner had already fixed exactly this and says so in a comment; the iOS runner had not. WHAT IS NOT CLAIMED, AND WHY THIS ROW IS OPEN RATHER THAN FIXED. Neither lane is wired and neither is recorded PASSED. The corrected mcp-phase-non-model command was run locally and its cross-adapter lane FAILED -- the electron leg timed out after 506s waiting for the Settings pane to report Connected -- where the same lane passed in 38s earlier the same day on the same code. That failure is NOT attributed to any change here: the dedicated cross-adapter-phase CI job runs the same four legs and passed at this exact revision, so the electron leg works. What it means is that the local result is order- or warmth-dependent and I do not yet know why, and a lane may not be recorded PASSED on a command whose only local run failed. TWO FURTHER LOCAL ATTEMPTS MADE THAT WORSE, NOT BETTER, and are recorded so nobody repeats them: re-running the cross-adapter lane alone failed 3 of its 4 legs (electron timeout, an iPhone Swift test, and an mcp out-of-band call refused as task_not_found), and a run with KEEPLING_E2E_POSTGRES_PORT overridden to this machine's spare port failed every lane in under a second because nothing was listening there -- that gate provisions its own database and must NOT be given that override. The variable that is unreliable here is this workstation, not the corrected command. TO CLOSE: wire both jobs, and produce one green run of each corrected command on CI -- which is the authority for both lanes anyway. The iOS subset additionally needs a job timeout above 50 minutes: its accessibility lane alone took 44 minutes locally. OWNER: BACKLOG. CLOSED 2026-09-15. Both conditions met at the SAME commit, 6003540. ios-simulator: CI run 34996420244 reports lanes=22 failed=0 blocked=0 excluded=2 across 463 executed cases, plus the command's second invocation (--lane server-driven-sim) PASS cases=4; the two EXCLUDED lines name device and server-driven-device with evidence_owned_by=ios-device, so the narrowed run is legible AS narrowed. mcp-phase-non-model: CI run 34996420275's job reports lanes=5 failed=0 blocked=0 excluded=1 across 237 cases (deterministic 194, adversarial 16, cross-adapter 16, simulated-client 9, protocol 2), with representative-model EXCLUDED to mcp-representative-model. The iOS job carries timeout-minutes: 150; its accessibility lane alone took 1782949 ms on the hosted runner, so the 50 this row warned against would indeed have killed it. GETTING HERE COST SEVEN COMMITS AND FIVE RED CI RUNS, AND NOT ONE DEFECT WAS IN THE CORRECTED COMMANDS. The first wired iOS run reported failed=21 of 22 lanes. What that number actually measured, in order of discovery: (1) three phase-gate runners killed their own children at Node's 1 MiB spawnSync maxBuffer default and reported the kill as a lane failure -- 15 of the 21; (2) the server-driven runner's -only-testing named the whole ServerDrivenTests CLASS, pulling in two cross-adapter RPC entry points that XCTFail without their KEEPLING_LANE_* parameters, so 4 passing scenarios reported as 6/2f; (3) two Swift tests hardcoded 13 contract vector files against a directory 05-11 and 06-08 had grown to 16, and one of them demanded ZERO wire-DTO-decodable objects where mcp-tools.json now carries 6 -- that assertion was a TRIPWIRE for an expired deviation, so it was replaced with the round-trip coverage D-14 asked for rather than having its threshold raised from 0 to 6; (4) UI-lane failures arrived with no assertion at all, a head-100/tail-200 excerpt dropping the .swift:NN: error: line into the elided middle of an XCUITest activity log; (5) a typed non-Latin title read back as 24 of 40 scalars on the hosted runner and verbatim locally -- a read outrunning typeText's slow path for scalars the hardware keyboard cannot produce; (6) the capture sheet's .dynamicType audit throwing an ENGINE-level error at 48 scalars, where 04-14 had measured and disclosed the same limitation for 512-scalar screens only. THE PATTERN IS THE ACTUAL FINDING: every one of those predates this window and not one is a regression. They accumulated in lanes nothing executed. The shape appeared a seventh time in the same session in an unrelated file -- --requirements in BOTH gates read the traceability table with .find(...), singular, against a table carrying one row per requirement, and so declared IOS-02..04 and MCP-02..05 absent from rows the table devotes one each to; no workflow job invokes --requirements, which is exactly why a broken id reader survived. An unwired check does not stay neutral. It rots silently and the whole bill comes due the day someone wires it. That deserves its own row rather than a clause in this one. NOT CLAIMED. The 463 simulator cases are the ios-simulator lane's evidence and nothing more. device and server-driven-device remain ios-device's, attested locally this session at 29 and 4 cases on a physical iPhone against a devicectl-installed signed build, and are excluded from this run BY NAME. representative-model remains BLOCKED on a paid credential and is excluded by name. The accessibility lane FAILED at 4bd7648 and 78fee8b and PASSED at a700ec9, 039ee8b, 3ad863a and 6003540 with an unchanged input_digest of bcb6a95277167611 -- identical inputs, different verdicts -- so its green here is one sample, not a proof of stability; four consecutive passes lower the estimated rate without retracting the observation. The capture sheet's .dynamicType narrowing is a disclosed engine limitation, not a fix, recorded in docs/testing/ios-testing.md.",
    "status": "fixed",
    "reason": "",
    "recorded_at": "2026-09-14T22:40:00.000Z",
