defmodule KeeplingWeb.TestFaultTest do
  ...
  setup do
    previous = System.get_env("KEEPLING_TEST_FIXTURE_TOKEN")
    System.put_env("KEEPLING_TEST_FIXTURE_TOKEN", @credential)

    on_exit(fn ->
      if previous do
        System.put_env("KEEPLING_TEST_FIXTURE_TOKEN", previous)
      else
        System.delete_env("KEEPLING_TEST_FIXTURE_TOKEN")
      end
    end)
  end

  test "before-acceptance fault drops the request before downstream work" do
    conn = fault_conn("before_acceptance")

    assert_raise TestFaultController.ConnectionDropped, "test connection dropped", fn ->
      TestFaultController.call(conn, [])
    end
  end
  ...
  test "fault controls are credentialed, closed, and privacy-safe" do
    conn =
      ...
      |> put_req_header("x-keepling-test-fixture", "wrong")
  ...
  test "router references the fault plug only inside the test compile gate" do
    ...
    refute source =~ ~r/plug KeeplingWeb\.TestFaultController[\s\S]*scope "\/api\/v1"/
  end

  defp fault_conn(mode) do
    conn(:post, "/api/v1/commands/complete-task")
    |> put_req_header("x-keepling-test-fault", mode)
    |> put_req_header("x-keepling-test-fixture", @credential)
  end
end
