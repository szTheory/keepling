...
| A4 | VoiceOver: KNOWN GAP | Trigger the "Discard unsaved changes?" dialog (edit a task, then navigate away before saving). | **Expected to fail** — this dialog does not move focus into itself (`accessibility.spec.ts`'s "DISCLOSED GAP" test proves this automatically). Record what VoiceOver actually does here (likely: continues announcing whatever was focused before, not the dialog) — this is real, disclosed evidence, not a defect to silently work around. |
...
- The initial physical accessibility pass has zero unresolved release-blocking defects (a
  disclosed, non-blocking gap like A4 is acceptable and must be recorded, not hidden).
