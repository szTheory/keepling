...
| `server-driven-device` | `node tooling/verify-ios-phase.mjs --lane server-driven-device` | `KeeplingCoreTests` (`ServerDrivenTests`) | The same four server-driven scenarios as `server-driven-sim`, driven on a PHYSICAL iPhone against real Phoenix on real PostgreSQL. The proxy is served from this Mac's tailnet MagicDNS name under a real Let's Encrypt certificate, so the phone validates it with the SHIPPING trust path — default `URLSessionTransport`, production `KeeplingSyncAdapter` constructor, production HTTPS guard, no test-only trust code on any configuration. Refuses to report `PASS` unless the run actually used `transport=https-publicly-trusted`: a silent fallback to a weaker transport reports `BLOCKED` (D-24) |
| `device` | `node tooling/verify-ios-phase.mjs --lane device` | `KeeplingUITests` (`DeviceCoreLoopTests`, `DeviceRecoveryTests`, `AccessibilityAuditTests`, `DynamicTypeSnapshotTests`, four `SyncStateMatrixTests` state cases), `StorageTests` (`DataProtectionTests`) | Everything that only physical hardware can answer, against a `devicectl`-installed signed build whose identity is read back out of the running process and refused on mismatch BEFORE any test runs (D-21): the daily loop, offline recovery, the no-notice SIGKILL and relaunch, at-rest protection class where iOS actually enforces Data Protection, the one device confirmation run of the Plan 04-13 accessibility suites, and IOS-04's five sync states told apart by exact copy and named recovery action. Chained behind a lock-state probe, so a locked phone reports `BLOCKED` rather than a confusing xcodebuild timeout |
...
**Plan 04-16 has since run, and Plan 04-18 closed what it left open.** The paid Apple
Developer Program membership is active, `DEVELOPMENT_TEAM` resolves through a
gitignored `Signing.xcconfig` include, the device is registered, and the `device`
lane reports `PASS` against a `devicectl`-installed signed build bound to its
attested build digest. Disclosures below that still say "**Still open**" are the
ones physical hardware genuinely cannot answer; the ones 04-16 and 04-18 closed
say so explicitly and name the suite that closed them.
...
| 2 | G7 at-rest data protection: the store's protection class is `.completeUntilFirstUserAuthentication`, and a background write while locked does not fail with an I/O error or `0xdead10cc` (D-04) | The simulator lane (`DataProtectionTests`) proves the requested protection class reads back correctly via `URLResourceValues` (not `FileManager.attributesOfItem`, which reads back `nil` on the Simulator host filesystem — an empirically confirmed false-negative trap) and that the durable unit is fully excluded from device backup. | The iOS Simulator enforces no real Data Protection restriction — no simulator write is ever blocked or delayed by lock state. Whether a real locked-device background write avoids `SQLITE_IOERR`/`0xdead10cc` can only be observed on physical hardware; the `device` lane has since run `DataProtectionTests` on physical hardware, and the enforceable-protection-class half is **now proven there** (`testStoreProtectionClassOnPhysicalDeviceIsEnforceable`, with `devicectl device info lockState` confirming a passcode is actually set, so the class is enforceable rather than nominal). The LOCKED-WRITE half is **still open**, for a specific checkable reason: nothing available can lock the phone under program control — `devicectl` exposes `info lockState` (a read) but no `lock` verb, and `XCUIDevice`'s lock control lives in the UI-testing framework, which the `StorageTests` unit bundle does not link. Driving it from the UI-test target would require a new app-side `protectedDataWillBecomeUnavailable` write hook — new production surface added solely to make a gate pass. `testBackgroundWriteWhileLockedDoesNotTakeAnIOErrorOrTerminate` records that reason, never a bare `XCTSkip`. |
| 3 | D-22 Criterion 2: durable state survives OS-terminated processes with no notice given beforehand | No lane in this codebase induces a real jetsam (OS-initiated termination under memory pressure) — no test target can manufacture real memory pressure or call the private API that simulates it. Plan 04-16 was to prove the STRICTER equivalent case (`SIGKILL`, which a jetsam termination is itself implemented as from the process's own point of view) on a real device, followed by a relaunch that restores correctly from the durable outbox. | Real jetsam under real memory pressure specifically — it cannot be requested on demand by any test target. The stricter proxy **has now run on physical hardware**: `DeviceRecoveryTests` delivers an out-of-process SIGKILL (`tooling/ios-device/hard-kill.mjs`) and proves an offline mutation survives kill-and-relaunch, that relaunch does not duplicate it, and that an unsent capture draft survives. SIGKILL cannot be caught and runs no shutdown path at all, whereas a jetsam kill may follow a clean suspension; the weaker case is not claimed under the stronger one's evidence. The SERVER-DRIVEN half of this criterion — authentication expiry, account fencing, duplicate replay, structured conflict, injected server-side — is **also now proven on hardware** by the `server-driven-device` lane. |
...
| 6 | D-22 Criterion 4: `VoiceOver`, Dynamic Type, Reduce Motion, and focus safety meet WCAG 2.2 AA and never strand assistive-technology focus | `performAccessibilityAudit` (all seven types) on every screen in the closed `ScreenInventory`, the accessibility Dynamic Type matrix (five accessibility categories on one representative screen, full inventory at default and largest), Differentiate Without Color, the single `Motion.swift` Reduce Motion gate with a structural no-animation-escapes-it scan, and `FocusSafetyTests`' application-logic proof (`lastRequestedFocusTarget`) that the app decided and requested the correct next-focus target after row removal and sheet dismissal. | **`VoiceOver`'s actual synthesized speech output** — no test plays or transcribes audio. **Real screen-reader gesture navigation** — XCUITest drives the accessibility tree directly, not the two-finger-swipe rotor gestures a real VoiceOver/Switch Control user performs. **Whether the OS actually lands VoiceOver's focus** after row removal or sheet dismissal — measured directly during 04-13: `XCUIElement.hasFocus` never reported `true` for any `@AccessibilityFocusState`-bound element in the Simulator, for either a row or a toolbar button, regardless of correct wiring; `@AccessibilityFocusState` only retains a requested value once a real assistive-technology client confirms the move landed, which this harness cannot drive. The one physical-device confirmation run **has now happened**: the `device` lane executes `AccessibilityAuditTests` and `DynamicTypeSnapshotTests` on hardware, and that run found a real device-only contrast defect the simulator never reproduced. What remains open is narrower and is a property of the harness, not of the app: VoiceOver's synthesized speech, real rotor/gesture navigation, and whether the OS itself lands focus. No lane will ever assert those. |
| 7 | D-22 Criterion 5: the supported daily loop stays available on Jon's phone for sustained daily use, without a recurring manual reinstall | Not automatable at all, by design (04-16-PLAN.md's own flagged assumption). The closest automatable proxy Plan 04-16 was to deliver: the loop installed on the physical device under a non-expiring provisioning profile, with every device-lane case passing against that installed build. | Sustained daily adoption is owner dogfood feedback, never a gate — no lane in this codebase asserts it and none ever will. The automatable proxy **has been produced**: `tooling/build-ios-signed.mjs` signs and installs the loop on the phone under the paid-membership profile, and every device-lane case passes against that installed build, bound to its attested digest. |
| 9 | IOS-01's daily loop, including UNDO, on physical hardware | `DeviceCoreLoopTests` drives capture, view, open, edit, complete, reopen, trash and restore on a physical iPhone; undo is proven on the simulator by `UndoReconciliationTests` (reducer/store) and `UndoPersistenceTests` (the control's persistence, labelling, both-tab overflow placement, and exception arbitration). | **UNDO on hardware.** No device suite drives it, and not by oversight: undo availability is minted by a SERVER acknowledgement carrying an undo handle (`WorkspaceFacade.undoAvailability` is fed from `acknowledge(_:)`), so a local trash in a device lane with no server produces no undo affordance to tap. The simulator lane injects availability with `KEEPLING_UITEST_UNDO_AVAILABLE`. Closing this means running the UI device lane against the recording-proxy stack the way `server-driven-device` already does for the adapter. **Open**, and found by 04-18's verification rather than claimed as done. |
...
- **2 device-bound lanes**, required by IOS-01, IOS-02, IOS-04, and the
  SRV-02 iPhone adapter proof, both now reporting `PASS` against a
  `devicectl`-installed signed build whose identity is read back out of
  the running process and refused on mismatch before any test executes:
  `device` (the daily loop, offline recovery, no-notice SIGKILL and
  relaunch, at-rest protection class, the accessibility confirmation run,
  and IOS-04's five sync states) and `server-driven-device` (the four
  server-driven scenarios against real Phoenix behind the recording
  proxy, over a publicly-trusted tailnet host). The aggregate gate
  (`node tooling/verify-ios-phase.mjs`) therefore reports overall success
  on real hardware. **Disclosed residual:** the device suites build
  `Debug`, because a `Release` build strips the XCTest hosting the
  physical-device suites require. That substitution is stated and
  attested rather than silent — `BuildAttestation.configuration` is
  compiled in from a `#if`, the console line carries it, and
  `attestation.mjs --expect-configuration` refuses a mismatch. Proving
  the `Release` binary's behaviour on hardware remains open.
